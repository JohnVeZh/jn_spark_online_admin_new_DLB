...
<%@page import="com.business.PreferentialCodeGenerate.PreferentialCodeGenerate"%>
<%
...
<link rel="stylesheet" type="text/css" id="theme"/>
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="PreferentialCodeGenerate.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(idVal) {
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="PreferentialCodeGenerate.do?act=realdeleteById&idVal="+idVal;
						listForm.submit();
					}
				)
			}
		...
		//添加,生成券码
		function preAdd(pcId){
			if(pcId!=""){
				top.Dialog.open({URL:"<%=path%>/business/PreferentialCodeGenerate.do?act=preAdd&pcId="+pcId,ID:"a1",Width:1024,Height:768,Title:"生成券码"});
			} 
		}
		//修改
		function preUpdate(j) {
			var idVal = isSel();
			if(idVal!=""){
			  listForm.action="PreferentialCodeGenerate.do?act=preUpdate"+idVal;
			  listForm.submit();
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="PreferentialCodeGenerate.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		...
		<form name="listForm" scope="request" action="<%=path%>/business/PreferentialCodeGenerate.do?act=list" method="post">
			<input type="text" name="pcId" id="pcId" value="${pcId }" style="display: none"/>
			<div class="box2" panelTitle="功能面板" roller="false">
				<table style="width:100%">
				<tr>
					<td><a href="javascript:;" onclick="preAdd('${pcId }')" title="添加"> <span
							class="img_add"></span>
					</a>
					<a id="del_mfp"
	...
	{
		po = (PreferentialCodeGenerate)list.get(i);
%>
	<tr  >
    	<td align="center"><input type="checkbox" name="id" value="<%=po.getId()%>" onclick="event.cancelBubble=true;"></td>
        <td align="center"><%=++sn%></td>
		<td class="DataTable_Content" align="left" title="<%= po.getPcgCode() %>"><%= po.getPcgCode() %></td>								
