...
<%@page import="com.business.JQrNwt.JQrNwt"%>
<%
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JQrNwt.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(idVal) {
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JQrNwt.do?act=realdeleteById&idVal="+idVal;
						listForm.submit();
					}
				)
			}
		...
		//添加
		function preAdd(){  
			top.Dialog.open({URL:"<%=path%>/business/JQrNwt.do?act=preAdd",ID:"a1",Width:1024,Height:768,Title:"新增"}); 
		}
		//修改
		function preUpdate(idVal) {
			if(idVal!=""){
				top.Dialog.open({URL:"<%=path%>/business/JQrNwt.do?act=preUpdate&id="+idVal,ID:"a2",Width:1024,Height:768,Title:"修改"}); 
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="JQrNwt.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		<div class="position">
			<div class="center">
				<div class="left">
					<div class="right">
					<span>当前位置：网课视频类型<!--< % out.write( MenuMgr.getMenuNavigation("402890e31094bb9c011094c692ea0003"));%>--></span>
					</div>
				</div>
			</div>
		</div>
		<form name="listForm" scope="request" action="<%=path%>/business/JQrNwt.do?act=list" method="post">
...
%>
	<tr  >
		...
		<td class="DataTable_Content" align="left">
			 <a href="javascript:;"
				onclick="preUpdate('<%=po.getId()%>')" title="删除"> <span class="img_edit"></span>
			</a>
			<a id="del_mfp"
