...
<script type="text/javascript" src="<%=path%>/libs/js/form/datePicker/WdatePicker.js"></script>
	...
	<script type="text/javascript">
		...
		function addProduct(idVal) {
			if(idVal!=""){
			 ...
			 $.post("<%=path%>/business/TbCouponProduct.do?act=addProduct",{pId:idVal,pcId:pcId,type:2},function(data){
               if(data.result){
		...
		function addProducts() {
			...
			for(var i = 0; i <inps.length; i++){
			 ...
			 var pcId = $("#pcId").val();
			$("#scrollContent").mask("表单提交中！");
			 $.post("<%=path%>/business/TbCouponProduct.do?act=addProducts",{pId:idVal,pcId:pcId,type:2},function(data){
               if(data.result){
				   reload("TbCouponProduct.do?act=list");
                   window.location.reload(true,function () {
					   $("#scrollContent").unmask();
	...
	<div id="scrollContent">
		<form name="listForm" scope="request" action="<%=path%>/business/TbCouponProduct.do?act=p_list" method="post">
