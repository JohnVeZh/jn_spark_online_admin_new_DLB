...
<%@page import="com.business.JCommonType.JCommonType"%>
<%
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JCommonType.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(idVal) {
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JCommonType.do?act=realdeleteById&idVal="+idVal;
						listForm.submit();
					}
				)
			}
		...
		//添加
		function preAdd(){    
			top.Dialog.open({URL:"<%=path%>/business/JCommonType.do?act=preAdd",ID:"a1",Width:1024,Height:768,Title:"新增"});     
		}
		//修改
		function preUpdate(idVal) {
			if(idVal!=""){
				top.Dialog.open({URL:"<%=path%>/business/JCommonType.do?act=preUpdate&id="+idVal,ID:"a2",Width:1024,Height:768,Title:"修改"});     
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="JCommonType.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		<div class="position">
			<div class="center">
				<div class="left">
					<div class="right">
					<span>当前位置：词汇类别<!--< % out.write( MenuMgr.getMenuNavigation("402890e31094bb9c011094c692ea0003"));%>--></span>
					</div>
				</div>
			</div>
		</div>
		<form name="listForm" scope="request" action="<%=path%>/business/JCommonType.do?act=list" method="post">
			...
			<table  class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false" id="listTable">
					...
					<tr >
						...
						<th height="25"  align="center" class="DataTable_Field" title="类别名称 ">名称</th>								 
