...
<%@page import="com.business.NetworkVideoTeaher.NetworkVideoTeaher"%>
<%
...
<script type="text/javascript" src="<%=path%>/js/imgs.js"></script>
	 ...
	 top.Dialog.close();
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="NetworkVideoTeaher.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(id) {
			if(id!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="NetworkVideoTeaher.do?act=realdeleteById&tId="+id;
						listForm.submit();
					}
				)
			}
		...
		//添加
		function preAdd(){
		   top.Dialog.open({URL:"<%=path%>/business/NetworkVideoTeaher.do?act=preAdd",ID:"a1",Width:1024,Height:768,Title:"新增"})       
		}
		//修改
		function preUpdate(id) {
			if(id!=""){
			top.Dialog.open({URL:"<%=path%>/business/NetworkVideoTeaher.do?act=preUpdate&id="+id,ID:"a1",Width:1024,Height:768,Title:"修改"})       
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="NetworkVideoTeaher.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		...
		<form name="listForm" scope="request" action="<%=path%>/business/NetworkVideoTeaher.do?act=list" method="post">
	...
	{
		po = (NetworkVideoTeaher)list.get(i);
%>
	<tr  >
        ...
        %>
        	<div style="float: center;padding: 5px 5px 5px 5px">
			   <a href="javascript:thumbImgsDiv('<%=path%>/<%=po.getImgpath()%>',0)" >
			      <img src="<%=path%>/<%= po.getImgpath()%>" width="90px" height="90px" style="border:1px solid #ccc;padding:5px;"/>
			   </a>
			 </div>
        <%
        	}
		...
		<td class="DataTable_Content" align="left" title="<%= po.getCreatetiem() %>"><%= po.getCreatetiem() %></td>	
