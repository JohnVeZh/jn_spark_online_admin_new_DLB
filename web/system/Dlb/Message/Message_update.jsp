						...
						<table class="tableStyle" transMode="true" footer="normal">
							...
							<tr>
								...
								<td width="40%" align="left">
									<input type="text" name="title" class="validate[required] iptClass" required="required" value="${message.title}" />
									<span class="star"></span>
								</td>
								...
								<td width="40%" align="left">
									<input type="text" name="intro" class="validate[required] iptClass" required="required" value="${message.intro}" />
									<span class="star"></span>
								</td>
							...
							<tr>
								...
								<td width="40%" align="left">
									...
									<select name="jumpType" id="jumpType"  style="z-index: 9999" onchange="changeJumpType()"  >
										...
										<c:choose>
											<c:when test="${message.jumpType==3}">
												<option value="3" selected="selected" >首页</option>
											</c:when>
											<c:otherwise><option value="3"  >首页</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==4}">
												<option value="4" selected="selected" >网课-四级</option>
											</c:when>
											<c:otherwise><option value="4"  >网课-四级</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==5}">
												<option value="5" selected="selected" >网课-六级</option>
											</c:when>
											<c:otherwise><option value="5"  >网课-六级</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==6}">
												<option value="6" selected="selected" >网课-考研</option>
											</c:when>
											<c:otherwise><option value="6"  >网课-考研</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==7}">
												<option value="7" selected="selected" >社区-资讯</option>
											</c:when>
											<c:otherwise><option value="7"  >社区-资讯</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==8}">
												<option value="8" selected="selected" >社区-活动</option>
											</c:when>
											<c:otherwise><option value="8"  >社区-活动</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==9}">
												<option value="9" selected="selected" >图书</option>
											</c:when>
											<c:otherwise><option value="9"  >图书</option></c:otherwise>
										</c:choose>
										<c:choose>
											<c:when test="${message.jumpType==10}">
												<option value="10" selected="selected" >我的</option>
											</c:when>
											<c:otherwise><option value="10"  >我的</option></c:otherwise>
										</c:choose>
								...
								<td width="40%" align="left">
									<input type="text" id="contentId" name="contentId" class="validate[] iptClass" value="${message.contentId}" />
								</td>
							...
							<tr>
								...
								<td width="40%" align="left">
									<input placeholder="不填则立即发送" type="text" name="pushTimeStr" id="pushTimeStr" class="date validate[]" dateFmt="yyyy-MM-dd HH:mm:ss" value="<fmt:formatDate value="${message.pushTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
								</td>
								...
								<td width="90%" align="left" colspan="3">
									<div style=" height:450px; overflow:auto; ">
										 <textarea  id="ue_introduce" name="content" style="width: 100%"  >${message.content}</textarea>
									</div>
								</td>
							</tr>
