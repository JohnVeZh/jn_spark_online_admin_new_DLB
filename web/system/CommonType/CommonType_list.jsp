 ...
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
		...
		function preAdd(){     
		    ...
		    if(parentid != ""){  
		    top.Dialog.open({URL:"<%=path%>/business/CommonType.do?act=preAdd&parentid="+parentid,ID:"a1",Width:1024,Height:768,Title:"新增"});
		   }
		}
		//修改
		function preUpdate(id) {
			if(id !=""){
			top.Dialog.open({URL:"<%=path%>/business/CommonType.do?act=preUpdate&id="+id,ID:"a1",Width:1024,Height:768,Title:"修改"});
			}
		}
		//详情
		function view(id) {
			if(id!=""){
			top.Dialog.open({URL:"<%=path%>/business/CommonType.do?act=view&id="+id,ID:"a1",Width:1024,Height:768,Title:"查看"});
			}
		}
		//清空查询数据
		function qing(){
		...
		<div class="position">
			<div class="center">
				<div class="left">
					...
					<span>当前位置：词汇类别管理<!--< % out.write( MenuMgr.getMenuNavigation("402890e31094bb9c011094c692ea0003"));%>--></span>
					</div>
				</div>
			</div>
		</div>
		<form name="listForm" scope="request" action="<%=path%>/business/CommonType.do?act=list" method="post">
			  ...
			  <input type="text" name="parentid" value="${parentid }" id="parentid" style="display: none"/>
				...
				<tr>
					<td><a href="javascript:;" onclick="preAdd('')" title="新增"> <span
						...
						onclick="doDels()"> <span class="img_delete"></span>
					</a>
			...
			<table  class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false" id="listTable">
				<tr >
					...
					<th height="25"  align="center" align="center" class="DataTable_Field" title=" * 类型：int 长度:10" >操作</th>								 
				</tr>
...
%>
	<tr  >
		...
		<a id="del_mfp" href="javascript:;" title="删除" onclick="doDelById('<%=po.getId()%>')"> 
