...
<%@page import="com.business.NewQuestionsType.NewQuestionsType"%>
<%
...
<script type="text/javascript">
	...
	function doDels() {
		...
		if (idVal != "") {
			top.Dialog.confirm("您确信要删除吗?", function() {
				listForm.action = "NewQuestionsType.do?act=realdelete" + idVal;
				listForm.submit();
			})
		}
	}
	...
	function doDelsById(id) {
		...
		if (id != "") {
			top.Dialog.confirm("您确信要删除吗?", function() {
				listForm.action = "NewQuestionsType.do?act=realdelete&id=" + id;
				listForm.submit();
			})
		}
	}
	...
	function preAdd() {
	   ...
	   if(parentId!=""&&levelType!=""){
	    top.Dialog.open({URL:"<%=path%>/business/NewQuestionsType.do?act=preAdd&parentId="+parentId+"&levelType="+levelType,ID:"a1",Width:1024,Height:768,Title:"新增"});
	   }
//		listForm.action = "NewQuestionsType.do?act=preAdd"
//		listForm.submit();
	}
	//修改
	function preUpdate(id) {
		//var idVal = isSel();
		if (id != "") {
		      top.Dialog.open({URL:"<%=path%>/business/NewQuestionsType.do?act=preUpdate&id=" + id,ID:"a1",Width:1024,Height:768,Title:"修改"});
//	        listForm.action = "NewQuestionsType.do?act=preUpdate&id=" + id;
//			listForm.submit();
		}
	}
	//详情
	function view() {
		var idVal = isSel();
		if (idVal != "") {
			listForm.action = "NewQuestionsType.do?act=view" + idVal;
			listForm.submit();
		}
	}
	//清空查询数据
	...
	<div id="scrollContent">
		<div class="position">
			<div class="center">
				<div class="left">
					<div class="right">
						<span>当前位置：新题型类型<!--< % out.write( MenuMgr.getMenuNavigation("402890e31094bb9c011094c692ea0003"));%>--></span>
					</div>
				</div>
			</div>
		</div>
		<form name="listForm" scope="request" action="<%=path%>/business/NewQuestionsType.do?act=list" method="post">
			<input type="text" name="parentId" value="${parentId }" id="parentId" style="display: none" /> 
			<input type="text" name="levelType" value="${levelType }" id="levelType" style="display: none" />
			<div class="box2" panelTitle="功能面板" roller="false">
				<table style="width:100%">
					<tr>
						<td><a href="javascript:;" onclick="preAdd('')" title="新增"> 
						     <span class="img_add"></span>
						</a>
						<a id="del_mfp" href="javascript:;" title="删除" onclick="doDels()">
						 <span class="img_delete"></span>
						</a>
							<!-- <div class="box_tool_line"></div> <input type="submit" value="查询" />&nbsp;&nbsp;
			...
			<table class="tableStyle" mode="list" useCheckbox="true"
				...
				<tr>
					...
					<th height="25" align="center" class="DataTable_Field" title="类别名称 ">类型名称</th>
				...
				<%
					...
					for (int i = 0; i < list.size(); i++) {
						po = (NewQuestionsType) list.get(i);
				%>
				<tr>
					...
					<td>
					 <a href="javascript:;" onclick="preUpdate('<%=po.getId()%>')" title="修改"> 
						...
						<a id="del_mfp" title="删除" href="javascript:;" onclick="doDelsById('<%=po.getId()%>')">
						 <span class="img_delete"></span>
						</a>
					</td>
				</tr>
