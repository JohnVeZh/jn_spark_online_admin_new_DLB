...
<!--框架必需end-->
		...
		<div class="position">
			...
			<div class="box2" panelTitle="功能面板" roller="false">
				<table style="width:100%">
					<tr>
						<td>
							<input type="button" value="添加上传" onclick="addfileClick()"/>
						</td>
					</tr>
				  </table>
				</div>
				...
				<div   style="width: 25%;float: left;">
					<div align="center">
						...
						<% if(po.getAlias().equals("CAROUSEL_PAD") || po.getAlias().equals("CAROUSEL_AN_PAD")){ %>
						            图片标题：<input type="text" id="typevalue<%= po.getId() %>" style="width: 230px" value="<%= po.getType() %>"/><br/>
						  	图片链接：<input type="text" id="nametext<%= po.getId() %>" style="width: 230px" value="<%= po.getName() %>"/>
						<% }else if(po.getAlias().equals("CAROUSEL_PHONE")){ %>
							图片标题：<input type="text" id="typevalue<%= po.getId() %>" style="width: 230px" value="<%= po.getType() %>"/><br/>
						 	图片链接：<input type="text" id="nametext<%= po.getId() %>" style="width: 230px" value="<%= po.getName() %>"/>
						<% }else{%>
							<%= po.getName() %>
					...
					<div align="center" style="padding-top:10px ">
					  ...
					  <input type="button" value="删除" onclick="doDelById('<%= po.getId() %>')"/>
					</div>
				</div>
				<%
					}
	    ...
	    if(data.result){
			$("#textadd").val(data.imgPath);
				var str = '<div style="width: 25%;float: left;padding:10px 10px 10px 10px">'
				    ...
				    + '</div>'
				   	  +'<div align="center" style="padding: 10px 10px 10px 10px">';
				     if(alias == 'CAROUSEL_PAD' || alias == 'CAROUSEL_AN_PAD' || alias == 'CAROUSEL_PHONE'){
				     		str = str+'图片标题：<input type="text" id="typevalue'+data.count+'" style="width: 230px" /><br/>';
						  	str = str+'图片链接：<input type="text" id="nametext'+data.count+'" style="width: 230px"/>';
						}else{
							str = str+'图片标题：<input type="hidden" id="typevalue'+data.count+'" style="width: 230px" /><br/>';
	...
	function addImg(path,count){
			...
			var typevalue = $("#typevalue"+count).val();
