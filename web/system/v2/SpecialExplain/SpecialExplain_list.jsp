...
<script type="text/javascript">
	...
	function batchDelete() {
		...
		if(idVal!=""){
			top.Dialog.confirm("您确信要删除吗?",
					function() {
						operForm.action="<%=path%>/business/training/explain.do?act=delete"+idVal;
						operForm.submit();
					}
			)
		}
	...
	function deleteById(id) {
		if(id!=""){
			top.Dialog.confirm("您确信要删除吗?",
				function() {
					operForm.action="<%=path%>/business/training/explain.do?act=delete&id="+id;
					operForm.submit();
				}
			)
		}
	...
	// 新增
	function preAdd(){
		top.Dialog.open({URL:"<%=path%>/business/training/explain.do?act=preAdd",ID:'special_explain_add',Width:1024,Height:420,Title:'新增'});
	}
	
	// 修改
	function preUpdate(id) {
		if(id !=""){
			top.Dialog.open({URL:"<%=path%>/business/training/explain.do?act=preUpdate&id="+id,ID:'special_explain_update',Width:1024,Height:420,Title:'修改'});
		}
	}
	...
	function view(id) {
		if(id != ""){
			top.Dialog.open({URL:"<%=path%>/business/training/explain.do?act=view&id="+id,ID:'special_explain_view',Width:1024,Height:768,Title:'查看'});
		}
	}
...
<div id="scrollContent">
	...
	<form name="listForm" scope="request" action="<%=path%>/business/training/explain.do?act=list" method="post">
		...
		<table  class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false" id="listTable">
			<tr >
				...
				<th height="25"  align="center" class="DataTable_Field" style="width:80px;">访问次数</th>
			...
			<c:forEach items='${lc.list}' var="item" varStatus="stat">
				<tr>
					...
					<td class="DataTable_Content" align="left" title='${item.visitNum}'>${item.visitNum}</td>
