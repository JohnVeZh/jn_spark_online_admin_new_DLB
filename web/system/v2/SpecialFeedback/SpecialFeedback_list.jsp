...
<script type="text/javascript">
	...
	function batchDelete() {
		...
		if(idVal!=""){
			top.Dialog.confirm("您确信要删除吗?",
					function() {
						operForm.action="<%=path%>/business/training/feedback.do?act=delete"+idVal;
						operForm.submit();
					}
			)
		}
	...
	function deleteById(id) {
		if(id!=""){
			top.Dialog.confirm("您确信要删除吗?",
				function() {
					operForm.action="<%=path%>/business/training/feedback.do?act=delete&id="+id;
					operForm.submit();
				}
			)
		}
	...
	function preUpdate(id) {
		if(id !=""){
			top.Dialog.open({URL:"<%=path%>/business/training/feedback.do?act=preUpdate&id="+id,ID:'special_feedback_update',Width:1024,Height:420,Title:'修改'});
		}
	}
...
<div id="scrollContent">
	...
	<form name="listForm" scope="request" action="<%=path%>/business/training/feedback.do?act=list" method="post">
					...
					<td>
						...
						<div style="float: left">
							<select name="isSolved" id="isSolved" >
								<option value="all">全部</option>
								<option value="0" <c:if test='${isSolved == "0"}'>selected="selected"</c:if>>未解决</option>
								<option value="1" <c:if test='${isSolved == "1"}'>selected="selected"</c:if>>已解决</option>
							</select>&nbsp;&nbsp;
						</div>
						<div style="float: left">
		...
		<table  class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false" id="listTable">
			<tr >
				...
				<th height="25"  align="center" class="DataTable_Field" style="width:200px">目录</th>
				<th height="25"  align="center" class="DataTable_Field" style="width:80px">是否解决</th>
				<th height="25"  align="center" class="DataTable_Field">问题描述</th>
			...
			<c:forEach items='${lc.list}' var="item" varStatus="stat">
				<tr>
					...
					<td class="DataTable_Content" align="left">${item.trainingCatalogName}</td>
					<td class="DataTable_Content" align="left" title='${item.section}'>
						<c:choose>
							<c:when test='${item.isSolved == "0"}'>未解决</c:when>
							<c:when test='${item.isSolved == "1"}'>已解决</c:when>
						</c:choose>
					</td>
					<td class="DataTable_Content" align="left" style="overflow: hidden;">
						${item.content}
					</td>
					...
					<td>
						<a href="javascript:void(0);" title="详情" onclick="preUpdate('${item.id}')">
