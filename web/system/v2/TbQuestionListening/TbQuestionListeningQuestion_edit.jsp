...
<link rel="stylesheet" type="text/css" id="theme"/>
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
        ...
        function submitForm() {
			listForm.action="TbQuestionListening.do?act=updateSubject"
			listForm.submit();
        }
		...
		function doDelOption(id){
		  if(id!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="TbQuestionListening.do?act=deleteOption&optionId="+id;
				   		listForm.submit()
					}
				)
			}
        ...
        function preAddOption(id) {
            if(id!=""){
                top.Dialog.open({URL:"<%=path%>/business/TbQuestionListening.do?act=preAddOption&questionId="+id,ID:"a001",Width:1024,Height:768,Title:"查看"});
            }
        }
	...
	<div id="scrollContent">
		...
		<form name="listForm" scope="request" action="<%=path%>/business/TbQuestionListening.do?act=updateSubject" method="post" onsubmit="return sub(this)">
					...
					<table class="tableStyle" transMode="true" footer="normal">
						<tr>
							...
							<td width="85%" align="left" colspan="2">
								<%--<input type="text" name="title" id="title" style="width: 95%" <c:if test="${ not empty questionForm.title}"> value="${questionForm.title}" </c:if> />--%>
								<textarea rows="1" cols="1" style="width: 80%" name="title" id="title" >
									<c:if test="${ not empty questionForm.title}"> ${questionForm.title} </c:if>
								</textarea>
							</td>
						...
						<tr>
							...
							<td width="85%" align="left">
								<input type="text" name="sortOrder" id="sortOrder" style="width: 70%" placeholder="只能是数字" value="${questionForm.sortOrder}"
			...
			<fieldset>
				...
				<table  class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false" id="listTable">
					...
					<%
						...
						TbQuestionListeningQuestionOption po = null;
						for (int i = 0; i < list.size(); i++)
						{
							po = (TbQuestionListeningQuestionOption) list.get(i);
					%>
					<tr  >
