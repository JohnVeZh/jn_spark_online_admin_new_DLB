...
<%@page import="com.business.JQrT.JQrT"%>
<%
...
<link rel="stylesheet" type="text/css" id="theme"/>
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JQrT.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(id) {
			if(id!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JQrT.do?act=realdeleteById&tId="+id;
						listForm.submit();
					}
				)
			}
		...
		//添加
		function preAdd(){       
			top.Dialog.open({URL:"<%=path%>/business/JQrT.do?act=preAdd",ID:"a1",Width:1024,Height:768,Title:"新增"});
		}
		//修改
		function preUpdate(id) {
			if(id!=""){
			top.Dialog.open({URL:"<%=path%>/business/JQrT.do?act=preUpdate&id="+id,ID:"a2",Width:1024,Height:768,Title:"修改"});
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="JQrT.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		...
		<form name="listForm" scope="request" action="<%=path%>/business/JQrT.do?act=list" method="post">
