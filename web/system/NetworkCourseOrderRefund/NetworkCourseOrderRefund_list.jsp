...
<script type="text/javascript">
	...
	function preUpdate(id) {
		if(id!=""){
			top.Dialog.open({URL:"<%=path%>/business/NetworkCourseOrderRefund.do?act=preUpdate&id="+id,ID:"a1",Width:910,Height:540,Title:"修改"});
		}
	}
	//详情
	function view(id) {
		if(id!=""){
			top.Dialog.open({URL:"<%=path%>/business/NetworkCourseOrderRefund.do?act=view&id="+id,ID:"a1",Width:400,Height:540,Title:"查看"});
		}
	}
	//发货
	function code(id) {
	...
	function exportExcel(){
		...
		lf1.action = "NetworkCourseOrderRefund.do?act=exportExcel";
		lf1.submit();
	}
...
<body>
<form name="lf1" style="display: none;" scope="request" action="<%=path%>/business/NetworkCourseOrderRefund.do?act=exportExcel" method="post">
...
<div id="scrollContent">
	...
	<form name="listForm" scope="request" action="<%=path%>/business/NetworkCourseOrderRefund.do?act=list" method="post">
						...
						<div style="float: left; " >
							...
							<div style="float: left;">
								<select name="StatrSel" selWidth="75" id="StatrSel" >
									...
									<option value="check_ing" <c:if test="${orderStatus=='check_ing'}"> selected  = "selected " </c:if> >退款审核中</option>
									<option value="check_passed" <c:if test="${orderStatus=='check_passed'}"> selected  = "selected " </c:if>  >退款中</option>
									<option value="pay_finished" <c:if test="${orderStatus=='pay_finished'}"> selected  = "selected " </c:if> >已退款</option>
		...
		<table  class="detailTable" mode="list" useCheckbox="true" trClick="false" selectRowButtonOnly="false" id="listTable">
			<tr >
				...
				<th width="4" height="25"  align="center" class="DataTable_Field" title="退款金额 ">退款金额</th>
            ...
            <c:if test="${empty mList}">
                <% out.println("<script>top.Dialog.alert('<h3 align=\"center\">木有找到相关记录'+'<br>'+'请重新输入筛选条件</h3>');</script>"); %>
			...
			<c:forEach items="${mList }" var="ml" varStatus="s">
				<tr >
					...
					<td class="DataTable_Content" align="left" title="${ml.fee }"><fmt:formatNumber value="${ml.fee }" pattern="0.00" /></td>
