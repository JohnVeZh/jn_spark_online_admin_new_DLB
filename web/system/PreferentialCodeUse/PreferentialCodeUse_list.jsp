...
<%@page import="com.business.PreferentialCodeUse.PreferentialCodeUse"%>
...
<link rel="stylesheet" type="text/css" id="theme"/>
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="PreferentialCodeUse.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(idVal) {
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="PreferentialCodeUse.do?act=realdeleteById&idVal="+idVal;
						listForm.submit();
					}
				)
			}
		...
		function preAdd(){    
			...
			top.Dialog.open({URL:"<%=path%>/business/PreferentialCodeUse.do?act=preAdd",ID:"a1",Width:1024,Height:768,Title:"新增"});
		   
		}
		//修改
		function preUpdate(idVal) {
			if(idVal!=""){
				top.Dialog.open({URL:"<%=path%>/business/PreferentialCodeUse.do?act=preUpdate&id="+idVal,ID:"a2",Width:1024,Height:768,Title:"编辑"});
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="PreferentialCodeUse.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		...
		<form name="listForm" scope="request" action="<%=path%>/business/PreferentialCodeUse.do?act=list" method="post">
			<input type="text" name="pcId" value="${pcId }" style="display: none"/>
	...
	{
		po = (PreferentialCodeUse)list.get(i);
%>
	<tr  >
		...
		<td class="DataTable_Content" align="left" >
			<div class="textSlice" title="<%= Tool.getValue("select pcg_code from preferential_code_generate where id='"+po.getPcgId()+"'") %>"><%= Tool.getValue("select pcg_code from preferential_code_generate where id='"+po.getPcgId()+"'") %></div>
		</td>								
		<td class="DataTable_Content" align="left" >
		...
		<td class="DataTable_Content" align="left" >
			<div class="textSlice" title="<%= po.getCreatetime() %>"><%= po.getCreatetime() %></div>
		</td>								
       <td align="left">
			<a id="del_mfp"
