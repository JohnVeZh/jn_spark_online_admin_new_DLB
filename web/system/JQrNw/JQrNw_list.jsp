...
<%@page import="com.business.JQrNw.JQrNw"%>
<%
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JQrNw.do?act=realdelete"+idVal;
						listForm.submit();
					}
				)
			}
		...
		function doDelsById(idVal) {
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
				   		listForm.action="JQrNw.do?act=realdeleteById&nwId="+idVal;
						listForm.submit();
					}
				)
			}
		...
		//添加
		function preAdd(nwtId){  
		 if(nwtId!=""){
			top.Dialog.open({URL:"<%=path%>/business/JQrNw.do?act=preAdd&nwtId="+nwtId,ID:"a1",Width:1024,Height:768,Title:"新增"}); 
		 }    
		}
		//修改
		function preUpdate(idVal) {
			if(idVal!=""){
				top.Dialog.open({URL:"<%=path%>/business/JQrNw.do?act=preUpdate&id="+idVal,ID:"a2",Width:1024,Height:768,Title:"修改"}); 
			}
		}
		//详情
		function view() {
			var idVal = isSel();
			if(idVal!=""){
		  	   listForm.action="JQrNw.do?act=view"+idVal;
		  		listForm.submit();
			}
		}
		//清空查询数据
	...
	<div id="scrollContent">
		<div class="position">
			<div class="center">
				<div class="left">
					<div class="right">
					<span>当前位置：网课视频管理<!--< % out.write( MenuMgr.getMenuNavigation("402890e31094bb9c011094c692ea0003"));%>--></span>
					</div>
				</div>
			</div>
		</div>
		<form name="listForm" scope="request" action="<%=path%>/business/JQrNw.do?act=list" method="post">
			<input type="text" name="nwtId" id="nwtId" value="${nwtId }" style="display: none;"/>
			<div class="box2" panelTitle="功能面板" roller="false">
				<table style="width:100%">
				<tr>
					<td><a href="javascript:;" onclick="preAdd('${nwtId }')" title="新增"> <span
							class="img_add"></span>
					</a>
					<a id="del_mfp"
			...
			<table  class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false" id="listTable">
					...
					<tr >
						...
						<th height="25"  align="center" class="DataTable_Field" title="图片路径 ">图片路径</th>								 
...
%>
	<tr  >
		...
		<td class="DataTable_Content" align="left">
			<% 
        		if(po.getImgPath()!=null&&!po.getImgPath().equals("")){
        	%>
        	<div style="float: left;padding: 5px 5px 5px 5px">
			    <a href="javascript:thumbImgsDiv('<%=path%>/<%=po.getImgPath()%>',0)" >
			      <img src="<%=path%>/<%= po.getImgPath()%>" width="82px" height="82px" style="border:1px solid #ccc;padding:5px;"/>
			   </a>
			 </div>
        	<% 
        		}
		...
		<td class="DataTable_Content" align="left">
			...
			<a id="del_mfp"
				href="javascript:;" onclick="doDelsById('<%=po.getId()%>')" title="删除"><span class="img_delete"></span> </a>
