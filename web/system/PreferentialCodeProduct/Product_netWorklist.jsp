 ...
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
		...
		function addProduct(idVal) {
			if(idVal!=""){
			 var pcId = $("#pcId").val();
			 $.post("<%=path%>/business/PreferentialCodeProduct.do?act=addProduct",{pId:idVal,pcId:pcId,type:1},function(data){  
               if(data.result){  
		...
		function addProducts() {
			...
			for(var i = 0; i <inps.length; i++){
			 ...
			 var pcId = $("#pcId").val();
			 $.post("<%=path%>/business/PreferentialCodeProduct.do?act=addProducts",{pId:idVal,pcId:pcId,type:1},function(data){  
               if(data.result){  
               	//top.frmright.window.location.reload();
		    	alert("添加成功，请手动刷新");
		    	top.Dialog.close();
	...
	<div id="scrollContent">
		<form name="listForm" scope="request" action="<%=path%>/business/NetworkVideo.do?act=pcp_list" method="post" >
