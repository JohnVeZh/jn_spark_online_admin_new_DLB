...
<link rel="stylesheet" type="text/css" id="theme"/>
 <!--3.3框架必需end-->
	...
	<script type="text/javascript">
		...
		function doDels() {
			...
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
						$.post("<%=path%>/business/PreferentialCodeProduct.do?act=realdelete",{idVal:idVal},function(data){  
			               if(data.result){  
					    	alert("删除成功，请手动刷新");
			               } 
			             });  
		...
		function doDelsById(idVal) {
			if(idVal!=""){
				top.Dialog.confirm("您确信要删除吗?",
				   	function() {
						 $.post("<%=path%>/business/PreferentialCodeProduct.do?act=realdeleteById",{idVal:idVal},function(data){  
			               if(data.result){  
					    	alert("删除成功，请手动刷新");
			               } 
			             });  
		...
		//添加
		function preAdd(){    
			top.Dialog.open({URL:"<%=path%>/business/PreferentialCodeProduct.do?act=preAdd",ID:"a1",Width:1024,Height:768,Title:"新增"});
		}
		//修改
		function preUpdate(idVal) {
			if(idVal!=""){
				top.Dialog.open({URL:"<%=path%>/business/PreferentialCodeProduct.do?act=preUpdate&id="+idVal,ID:"a2",Width:1024,Height:768,Title:"编辑"});
			}
		}
		//详情
		function view() {
		...
		function colloBook(id) {
			if(id!=""){
				if(${lmSize > 0}){
					alert("只能绑定一个商品，请先删除现有绑定商品。");
					return;
				}
		  		top.Dialog.open({URL:"<%=path%>/business/Product.do?act=pcp_list&pcId="+id,ID:'col1',Title:"配套图书",Width:1024,Height:768});
			}
		}
		//搭配网课
		function colloNetwork(id) {
			if(id!=""){
				if(${lmSize > 0}){
		  		...
		  		top.Dialog.open({URL:"<%=path%>/business/NetworkVideo.do?act=pcp_list&pcId="+id,ID:'col1',Title:"配套图书",Width:1024,Height:768});
			}
		}
	...
	<div id="scrollContent">
		...
		<form name="listForm" scope="request" action="<%=path%>/business/PreferentialCodeProduct.do?act=list" method="post">
			...
			<c:forEach items="${lm }" var="l" varStatus="s">
				<tr  >
					...
					<td class="DataTable_Content" align="left" >
						<div class="textSlice" title="${l.createtime }">${l.createtime }</div>
					</td>								
			       <td align="left">
						<a id="del_mfp"
