        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    //重新截取拼接图片的路径
                    var extStart = list[i].url.lastIndexOf("ueditor");
                    var ext = list[i].url.substring(extStart, list[i].url.length);
                    var imgUrl = getRootPath()+ "/" +ext;
                    
                    img.setAttribute('src', urlPrefix + imgUrl + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
