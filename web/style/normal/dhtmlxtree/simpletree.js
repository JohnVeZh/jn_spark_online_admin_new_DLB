...
function simpletreeInit(tree, dataUrl,smartLoadUrl,afterCall,onLoadingEnd)
{
    if (tree.smartLoad)
    {
...
//        tree.setChildCalcMode("leafs");
    ...
    function treeOnLoadStart(tree, id)
    {
        ...
        tree.insertNewChild(id, newId, "姝ｅ湪锷犺浇...", "", "loading.gif", "", "", 'SELECT');
...
function onButtonClick(menuitemId, itemId, context, tree)
{
    ...
    {
        if (window.confirm("纭畾瑕佸垹闄わ纸" + context.innerHTML + "锛夊悧锛?)) tree.deleteItem(itemId, false);
    }
    ...
    else if (menuitemId == "help_index") alert("鍚庢湡鎻愪緵锛?);
