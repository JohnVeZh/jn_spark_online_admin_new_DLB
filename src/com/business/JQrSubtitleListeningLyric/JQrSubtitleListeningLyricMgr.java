...
public class JQrSubtitleListeningLyricMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrSubtitleListeningLyric.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrSubtitleListeningLyricActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrSubtitleListeningLyric po = new JQrSubtitleListeningLyric();
	...
	*更新
	*/
	public void update(JQrSubtitleListeningLyricActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrSubtitleListeningLyric po = (JQrSubtitleListeningLyric) session.load(JQrSubtitleListeningLyric.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			JQrSubtitleListeningLyric po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrSubtitleListeningLyric) ses.load(JQrSubtitleListeningLyric.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrSubtitleListeningLyric(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrSubtitleListeningLyricActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrSubtitleListeningLyric po = (JQrSubtitleListeningLyric) ses.load(JQrSubtitleListeningLyric.class, ID);
			JQrSubtitleListeningLyricActionForm form = new JQrSubtitleListeningLyricActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrSubtitleListeningLyric as JQrSubtitleListeningLyric where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrSubtitleListeningLyric.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrSubtitleListeningLyric as JQrSubtitleListeningLyric where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrSubtitleListeningLyric> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrSubtitleListeningLyric as JQrSubtitleListeningLyric where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrSubtitleListeningLyric.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrSubtitleListeningLyric as JQrSubtitleListeningLyric where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
