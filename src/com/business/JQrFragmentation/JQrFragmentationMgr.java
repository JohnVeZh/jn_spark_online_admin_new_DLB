...
import com.business.JQrFragmentation.JQrFragmentation;
import com.business.JQrFragmentation.JQrFragmentationActionForm;


public class JQrFragmentationMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrFragmentation.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrFragmentationActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrFragmentation po = new JQrFragmentation();
	...
	*更新
	*/
	public void update(JQrFragmentationActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrFragmentation po = (JQrFragmentation) session.load(JQrFragmentation.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrFragmentation) ses.load(JQrFragmentation.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrFragmentation(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrFragmentationActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrFragmentation po = (JQrFragmentation) ses.load(JQrFragmentation.class, ID);
			JQrFragmentationActionForm form = new JQrFragmentationActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrFragmentation as JQrFragmentation where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrFragmentation.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrFragmentation as JQrFragmentation where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrFragmentation> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrFragmentation as JQrFragmentation where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrFragmentation.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrFragmentation as JQrFragmentation where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
