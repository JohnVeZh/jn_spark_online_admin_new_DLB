...
import com.business.JQrT.JQrTActionForm;


public class JQrTMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrT.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrTActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(JQrTActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrT po = (JQrT) session.load(JQrT.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrT) ses.load(JQrT.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrT(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrTActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrT po = (JQrT) ses.load(JQrT.class, ID);
			JQrTActionForm form = new JQrTActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrT as JQrT where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrT.sort ";
			Query qCount = ses
					.createQuery("select count(id) from JQrT as JQrT where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrT> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrT as JQrT where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrT.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrT as JQrT where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
