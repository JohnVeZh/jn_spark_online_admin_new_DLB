...
public class NetworkVideoMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NetworkVideo.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NetworkVideoActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NetworkVideo po = new NetworkVideo();
	...
	*更新
	*/
	public void update(NetworkVideoActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NetworkVideo po = (NetworkVideo) session.load(NetworkVideo.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NetworkVideo) ses.load(NetworkVideo.class, IDs[i]);
				//List list = ses.createFilter(po.getNetworkVideo(), "").list();
	 ...
	 * @throws Exception
	 */
	public NetworkVideoActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NetworkVideo po = (NetworkVideo) ses.load(NetworkVideo.class, ID);
			NetworkVideoActionForm form = new NetworkVideoActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NetworkVideo as NetworkVideo where NetworkVideo.isDel=0  "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NetworkVideo.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideo as NetworkVideo where NetworkVideo.isDel=0  "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NetworkVideo> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NetworkVideo as NetworkVideo where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NetworkVideo.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideo as NetworkVideo where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
