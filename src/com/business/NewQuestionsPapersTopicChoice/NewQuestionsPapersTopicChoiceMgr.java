...
public class NewQuestionsPapersTopicChoiceMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsPapersTopicChoice.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsPapersTopicChoiceActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsPapersTopicChoice po = new NewQuestionsPapersTopicChoice();
	...
	*更新
	*/
	public void update(NewQuestionsPapersTopicChoiceActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsPapersTopicChoice po = (NewQuestionsPapersTopicChoice) session.load(NewQuestionsPapersTopicChoice.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsPapersTopicChoice) ses.load(NewQuestionsPapersTopicChoice.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsPapersTopicChoice(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsPapersTopicChoiceActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsPapersTopicChoice po = (NewQuestionsPapersTopicChoice) ses.load(NewQuestionsPapersTopicChoice.class, ID);
			NewQuestionsPapersTopicChoiceActionForm form = new NewQuestionsPapersTopicChoiceActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsPapersTopicChoice as NewQuestionsPapersTopicChoice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsPapersTopicChoice.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopicChoice as NewQuestionsPapersTopicChoice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsPapersTopicChoice> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsPapersTopicChoice as NewQuestionsPapersTopicChoice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsPapersTopicChoice.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopicChoice as NewQuestionsPapersTopicChoice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
