...
public class ProductOrderLogisticsMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductOrderLogistics.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductOrderLogisticsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductOrderLogistics po = new ProductOrderLogistics();
	...
	*更新
	*/
	public void update(ProductOrderLogisticsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductOrderLogistics po = (ProductOrderLogistics) session.load(ProductOrderLogistics.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductOrderLogistics) ses.load(ProductOrderLogistics.class, IDs[i]);
				//List list = ses.createFilter(po.getProductOrderLogistics(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductOrderLogisticsActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductOrderLogistics po = (ProductOrderLogistics) ses.load(ProductOrderLogistics.class, ID);
			ProductOrderLogisticsActionForm form = new ProductOrderLogisticsActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductOrderLogistics as ProductOrderLogistics where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductOrderLogistics.id   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderLogistics as ProductOrderLogistics where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductOrderLogistics> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductOrderLogistics as ProductOrderLogistics where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductOrderLogistics.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderLogistics as ProductOrderLogistics where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
