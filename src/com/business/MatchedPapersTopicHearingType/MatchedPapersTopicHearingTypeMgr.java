...
public class MatchedPapersTopicHearingTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicHearingType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicHearingTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicHearingType po = new MatchedPapersTopicHearingType();
	...
	*更新
	*/
	public void update(MatchedPapersTopicHearingTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicHearingType po = (MatchedPapersTopicHearingType) session.load(MatchedPapersTopicHearingType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicHearingType) ses.load(MatchedPapersTopicHearingType.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicHearingType(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicHearingTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicHearingType po = (MatchedPapersTopicHearingType) ses.load(MatchedPapersTopicHearingType.class, ID);
			MatchedPapersTopicHearingTypeActionForm form = new MatchedPapersTopicHearingTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicHearingType as MatchedPapersTopicHearingType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicHearingType.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearingType as MatchedPapersTopicHearingType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicHearingType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicHearingType as MatchedPapersTopicHearingType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicHearingType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearingType as MatchedPapersTopicHearingType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
