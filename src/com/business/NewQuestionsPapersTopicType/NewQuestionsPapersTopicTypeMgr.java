...
public class NewQuestionsPapersTopicTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsPapersTopicType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsPapersTopicTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsPapersTopicType po = new NewQuestionsPapersTopicType();
	...
	*更新
	*/
	public void update(NewQuestionsPapersTopicTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsPapersTopicType po = (NewQuestionsPapersTopicType) session.load(NewQuestionsPapersTopicType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsPapersTopicType) ses.load(NewQuestionsPapersTopicType.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsPapersTopicType(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsPapersTopicTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsPapersTopicType po = (NewQuestionsPapersTopicType) ses.load(NewQuestionsPapersTopicType.class, ID);
			NewQuestionsPapersTopicTypeActionForm form = new NewQuestionsPapersTopicTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsPapersTopicType as NewQuestionsPapersTopicType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsPapersTopicType.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopicType as NewQuestionsPapersTopicType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsPapersTopicType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsPapersTopicType as NewQuestionsPapersTopicType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsPapersTopicType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopicType as NewQuestionsPapersTopicType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
