...
public class JQrMpthtMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrMptht.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrMpthtActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrMptht po = new JQrMptht();
	...
	*更新
	*/
	public void update(JQrMpthtActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrMptht po = (JQrMptht) session.load(JQrMptht.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrMptht) ses.load(JQrMptht.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrMptht(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrMpthtActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrMptht po = (JQrMptht) ses.load(JQrMptht.class, ID);
			JQrMpthtActionForm form = new JQrMpthtActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrMptht as JQrMptht where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrMptht.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrMptht as JQrMptht where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrMptht> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrMptht as JQrMptht where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrMptht.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrMptht as JQrMptht where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
