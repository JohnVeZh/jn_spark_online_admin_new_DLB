...
import com.business.PreferentialCodeGenerate.PreferentialCodeGenerate;
import com.business.PreferentialCodeGenerate.PreferentialCodeGenerateActionForm;


public class PreferentialCodeGenerateMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(PreferentialCodeGenerate.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(PreferentialCodeGenerateActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			PreferentialCodeGenerate po = new PreferentialCodeGenerate();
	...
	*更新
	*/
	public void update(PreferentialCodeGenerateActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			PreferentialCodeGenerate po = (PreferentialCodeGenerate) session.load(PreferentialCodeGenerate.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			PreferentialCodeGenerate po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (PreferentialCodeGenerate) ses.load(PreferentialCodeGenerate.class, IDs[i]);
				//List list = ses.createFilter(po.getPreferentialCodeGenerate(), "").list();
	 ...
	 * @throws Exception
	 */
	public PreferentialCodeGenerateActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			PreferentialCodeGenerate po = (PreferentialCodeGenerate) ses.load(PreferentialCodeGenerate.class, ID);
			PreferentialCodeGenerateActionForm form = new PreferentialCodeGenerateActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from PreferentialCodeGenerate as PreferentialCodeGenerate where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by PreferentialCodeGenerate.id   ";
			Query qCount = ses
					.createQuery("select count(id) from PreferentialCodeGenerate as PreferentialCodeGenerate where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<PreferentialCodeGenerate> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from PreferentialCodeGenerate as PreferentialCodeGenerate where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by PreferentialCodeGenerate.id desc";
			Query qCount = ses
					.createQuery("select count(id) from PreferentialCodeGenerate as PreferentialCodeGenerate where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
