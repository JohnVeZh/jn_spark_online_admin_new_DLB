...
public class SmsLogMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(SmsLog.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(SmsLogActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			SmsLog po = new SmsLog();
	...
	*更新
	*/
	public void update(SmsLogActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			SmsLog po = (SmsLog) session.load(SmsLog.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (SmsLog) ses.load(SmsLog.class, IDs[i]);
				//List list = ses.createFilter(po.getSmsLog(), "").list();
	 ...
	 * @throws Exception
	 */
	public SmsLogActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			SmsLog po = (SmsLog) ses.load(SmsLog.class, ID);
			SmsLogActionForm form = new SmsLogActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from SmsLog as SmsLog where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by SmsLog.addTime  desc ";
			Query qCount = ses
					.createQuery("select count(id) from SmsLog as SmsLog where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<SmsLog> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from SmsLog as SmsLog where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by SmsLog.id desc";
			Query qCount = ses
					.createQuery("select count(id) from SmsLog as SmsLog where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
