...
public class ProductCollocationMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductCollocation.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductCollocationActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductCollocation po = new ProductCollocation();
	...
	*更新
	*/
	public void update(ProductCollocationActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductCollocation po = (ProductCollocation) session.load(ProductCollocation.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductCollocation) ses.load(ProductCollocation.class, IDs[i]);
				//List list = ses.createFilter(po.getProductCollocation(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductCollocationActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductCollocation po = (ProductCollocation) ses.load(ProductCollocation.class, ID);
			ProductCollocationActionForm form = new ProductCollocationActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductCollocation as ProductCollocation where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductCollocation.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductCollocation as ProductCollocation where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductCollocation> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductCollocation as ProductCollocation where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductCollocation.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductCollocation as ProductCollocation where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
