...
import com.business.MatchedPapersTopicWriting.MatchedPapersTopicWritingActionForm;


public class MatchedPapersTopicWritingMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicWriting.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicWritingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicWriting po = new MatchedPapersTopicWriting();
	...
	*更新
	*/
	public void update(MatchedPapersTopicWritingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicWriting po = (MatchedPapersTopicWriting) session.load(MatchedPapersTopicWriting.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			MatchedPapersTopicWriting po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicWriting) ses.load(MatchedPapersTopicWriting.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicWriting(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicWritingActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicWriting po = (MatchedPapersTopicWriting) ses.load(MatchedPapersTopicWriting.class, ID);
			MatchedPapersTopicWritingActionForm form = new MatchedPapersTopicWritingActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicWriting as MatchedPapersTopicWriting where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicWriting.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicWriting as MatchedPapersTopicWriting where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicWriting> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicWriting as MatchedPapersTopicWriting where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicWriting.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicWriting as MatchedPapersTopicWriting where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
