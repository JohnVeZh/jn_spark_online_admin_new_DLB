...
public class MatchedPapersTopicHearingSubjectChoiceMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicHearingSubjectChoice.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicHearingSubjectChoiceActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicHearingSubjectChoice po = new MatchedPapersTopicHearingSubjectChoice();
	...
	*更新
	*/
	public void update(MatchedPapersTopicHearingSubjectChoiceActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicHearingSubjectChoice po = (MatchedPapersTopicHearingSubjectChoice) session.load(MatchedPapersTopicHearingSubjectChoice.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			MatchedPapersTopicHearingSubjectChoice po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicHearingSubjectChoice) ses.load(MatchedPapersTopicHearingSubjectChoice.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicHearingSubjectChoice(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicHearingSubjectChoiceActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicHearingSubjectChoice po = (MatchedPapersTopicHearingSubjectChoice) ses.load(MatchedPapersTopicHearingSubjectChoice.class, ID);
			MatchedPapersTopicHearingSubjectChoiceActionForm form = new MatchedPapersTopicHearingSubjectChoiceActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicHearingSubjectChoice as MatchedPapersTopicHearingSubjectChoice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicHearingSubjectChoice.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearingSubjectChoice as MatchedPapersTopicHearingSubjectChoice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicHearingSubjectChoice> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicHearingSubjectChoice as MatchedPapersTopicHearingSubjectChoice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicHearingSubjectChoice.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearingSubjectChoice as MatchedPapersTopicHearingSubjectChoice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
