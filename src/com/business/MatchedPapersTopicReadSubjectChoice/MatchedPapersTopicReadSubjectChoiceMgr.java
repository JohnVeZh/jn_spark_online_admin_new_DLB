...
public class MatchedPapersTopicReadSubjectChoiceMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicReadSubjectChoice.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicReadSubjectChoiceActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicReadSubjectChoice po = new MatchedPapersTopicReadSubjectChoice();
	...
	*更新
	*/
	public void update(MatchedPapersTopicReadSubjectChoiceActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicReadSubjectChoice po = (MatchedPapersTopicReadSubjectChoice) session.load(MatchedPapersTopicReadSubjectChoice.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			MatchedPapersTopicReadSubjectChoice po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicReadSubjectChoice) ses.load(MatchedPapersTopicReadSubjectChoice.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicReadSubjectChoice(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicReadSubjectChoiceActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicReadSubjectChoice po = (MatchedPapersTopicReadSubjectChoice) ses.load(MatchedPapersTopicReadSubjectChoice.class, ID);
			MatchedPapersTopicReadSubjectChoiceActionForm form = new MatchedPapersTopicReadSubjectChoiceActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicReadSubjectChoice as MatchedPapersTopicReadSubjectChoice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicReadSubjectChoice.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicReadSubjectChoice as MatchedPapersTopicReadSubjectChoice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicReadSubjectChoice> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicReadSubjectChoice as MatchedPapersTopicReadSubjectChoice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicReadSubjectChoice.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicReadSubjectChoice as MatchedPapersTopicReadSubjectChoice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
