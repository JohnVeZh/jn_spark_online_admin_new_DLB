...
public class JQrSubtitleListeningMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrSubtitleListening.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrSubtitleListeningActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(JQrSubtitleListeningActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrSubtitleListening po = (JQrSubtitleListening) session.load(JQrSubtitleListening.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrSubtitleListening) ses.load(JQrSubtitleListening.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrSubtitleListening(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrSubtitleListeningActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrSubtitleListening po = (JQrSubtitleListening) ses.load(JQrSubtitleListening.class, ID);
			JQrSubtitleListeningActionForm form = new JQrSubtitleListeningActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrSubtitleListening as JQrSubtitleListening where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrSubtitleListening.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrSubtitleListening as JQrSubtitleListening where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrSubtitleListening> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrSubtitleListening as JQrSubtitleListening where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrSubtitleListening.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrSubtitleListening as JQrSubtitleListening where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
