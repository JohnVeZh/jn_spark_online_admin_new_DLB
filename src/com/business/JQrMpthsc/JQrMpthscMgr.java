...
public class JQrMpthscMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrMpthsc.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrMpthscActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrMpthsc po = new JQrMpthsc();
	...
	*更新
	*/
	public void update(JQrMpthscActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrMpthsc po = (JQrMpthsc) session.load(JQrMpthsc.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrMpthsc) ses.load(JQrMpthsc.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrMpthsc(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrMpthscActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrMpthsc po = (JQrMpthsc) ses.load(JQrMpthsc.class, ID);
			JQrMpthscActionForm form = new JQrMpthscActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrMpthsc as JQrMpthsc where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrMpthsc.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrMpthsc as JQrMpthsc where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrMpthsc> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrMpthsc as JQrMpthsc where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrMpthsc.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrMpthsc as JQrMpthsc where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
