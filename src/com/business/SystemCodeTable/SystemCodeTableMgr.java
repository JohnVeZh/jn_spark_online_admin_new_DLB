...
import com.business.SystemCodeTable.SystemCodeTable;
import com.business.SystemCodeTable.SystemCodeTableActionForm;


public class SystemCodeTableMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(SystemCodeTable.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(SystemCodeTableActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			SystemCodeTable po = new SystemCodeTable();
	...
	*更新
	*/
	public void update(SystemCodeTableActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			SystemCodeTable po = (SystemCodeTable) session.load(SystemCodeTable.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (SystemCodeTable) ses.load(SystemCodeTable.class, IDs[i]);
				//List list = ses.createFilter(po.getSystemCodeTable(), "").list();
	 ...
	 * @throws Exception
	 */
	public SystemCodeTableActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			SystemCodeTable po = (SystemCodeTable) ses.load(SystemCodeTable.class, ID);
			SystemCodeTableActionForm form = new SystemCodeTableActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from SystemCodeTable as SystemCodeTable where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by SystemCodeTable.updatetime desc and SystemCodeTable.id   ";
			Query qCount = ses
					.createQuery("select count(id) from SystemCodeTable as SystemCodeTable where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<SystemCodeTable> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from SystemCodeTable as SystemCodeTable where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by SystemCodeTable.id desc";
			Query qCount = ses
					.createQuery("select count(id) from SystemCodeTable as SystemCodeTable where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
