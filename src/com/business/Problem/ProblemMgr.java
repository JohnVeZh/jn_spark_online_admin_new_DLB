...
public class ProblemMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(Problem.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProblemActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(ProblemActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			Problem po = (Problem) session.load(Problem.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (Problem) ses.load(Problem.class, IDs[i]);
				//List list = ses.createFilter(po.getProblem(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProblemActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			Problem po = (Problem) ses.load(Problem.class, ID);
			ProblemActionForm form = new ProblemActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from Problem as Problem where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by Problem.id   ";
			Query qCount = ses
					.createQuery("select count(id) from Problem as Problem where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<Problem> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from Problem as Problem where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by Problem.id desc";
			Query qCount = ses
					.createQuery("select count(id) from Problem as Problem where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
