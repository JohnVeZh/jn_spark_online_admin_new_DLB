...
public class NewQuestionsWrongQuestionMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsWrongQuestion.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsWrongQuestionActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsWrongQuestion po = new NewQuestionsWrongQuestion();
	...
	*更新
	*/
	public void update(NewQuestionsWrongQuestionActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsWrongQuestion po = (NewQuestionsWrongQuestion) session.load(NewQuestionsWrongQuestion.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewQuestionsWrongQuestion po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsWrongQuestion) ses.load(NewQuestionsWrongQuestion.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsWrongQuestion(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsWrongQuestionActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsWrongQuestion po = (NewQuestionsWrongQuestion) ses.load(NewQuestionsWrongQuestion.class, ID);
			NewQuestionsWrongQuestionActionForm form = new NewQuestionsWrongQuestionActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsWrongQuestion as NewQuestionsWrongQuestion where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsWrongQuestion.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsWrongQuestion as NewQuestionsWrongQuestion where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsWrongQuestion> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsWrongQuestion as NewQuestionsWrongQuestion where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsWrongQuestion.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsWrongQuestion as NewQuestionsWrongQuestion where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
