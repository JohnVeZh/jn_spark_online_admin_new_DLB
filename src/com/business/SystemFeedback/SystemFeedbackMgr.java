...
import com.business.SystemFeedback.SystemFeedback;
import com.business.SystemFeedback.SystemFeedbackActionForm;


public class SystemFeedbackMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(SystemFeedback.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(SystemFeedbackActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			SystemFeedback po = new SystemFeedback();
	...
	*更新
	*/
	public void update(SystemFeedbackActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			SystemFeedback po = (SystemFeedback) session.load(SystemFeedback.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			SystemFeedback po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (SystemFeedback) ses.load(SystemFeedback.class, IDs[i]);
				//List list = ses.createFilter(po.getSystemFeedback(), "").list();
	 ...
	 * @throws Exception
	 */
	public SystemFeedbackActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			SystemFeedback po = (SystemFeedback) ses.load(SystemFeedback.class, ID);
			SystemFeedbackActionForm form = new SystemFeedbackActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from SystemFeedback as SystemFeedback where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by SystemFeedback.id   ";
			Query qCount = ses
					.createQuery("select count(id) from SystemFeedback as SystemFeedback where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<SystemFeedback> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from SystemFeedback as SystemFeedback where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by SystemFeedback.id desc";
			Query qCount = ses
					.createQuery("select count(id) from SystemFeedback as SystemFeedback where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
