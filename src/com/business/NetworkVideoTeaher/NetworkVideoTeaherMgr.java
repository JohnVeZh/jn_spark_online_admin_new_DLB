...
public class NetworkVideoTeaherMgr extends AbstractHibernateDAO {
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NetworkVideoTeaherActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NetworkVideoTeaher po = new NetworkVideoTeaher();
	...
	*更新
	*/
	public void update(NetworkVideoTeaherActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NetworkVideoTeaher po = (NetworkVideoTeaher) session.load(NetworkVideoTeaher.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NetworkVideoTeaher po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NetworkVideoTeaher) ses.load(NetworkVideoTeaher.class, IDs[i]);
				//List list = ses.createFilter(po.getNetworkVideoTeaher(), "").list();
	 ...
	 * @throws Exception
	 */
	public NetworkVideoTeaherActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NetworkVideoTeaher po = (NetworkVideoTeaher) ses.load(NetworkVideoTeaher.class, ID);
			NetworkVideoTeaherActionForm form = new NetworkVideoTeaherActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NetworkVideoTeaher as NetworkVideoTeaher where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NetworkVideoTeaher.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoTeaher as NetworkVideoTeaher where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NetworkVideoTeaher> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NetworkVideoTeaher as NetworkVideoTeaher where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NetworkVideoTeaher.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoTeaher as NetworkVideoTeaher where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
