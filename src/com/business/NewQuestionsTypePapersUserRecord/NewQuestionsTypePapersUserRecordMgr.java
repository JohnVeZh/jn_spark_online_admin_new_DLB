...
public class NewQuestionsTypePapersUserRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsTypePapersUserRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsTypePapersUserRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsTypePapersUserRecord po = new NewQuestionsTypePapersUserRecord();
	...
	*更新
	*/
	public void update(NewQuestionsTypePapersUserRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsTypePapersUserRecord po = (NewQuestionsTypePapersUserRecord) session.load(NewQuestionsTypePapersUserRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewQuestionsTypePapersUserRecord po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsTypePapersUserRecord) ses.load(NewQuestionsTypePapersUserRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsTypePapersUserRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsTypePapersUserRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsTypePapersUserRecord po = (NewQuestionsTypePapersUserRecord) ses.load(NewQuestionsTypePapersUserRecord.class, ID);
			NewQuestionsTypePapersUserRecordActionForm form = new NewQuestionsTypePapersUserRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsTypePapersUserRecord as NewQuestionsTypePapersUserRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsTypePapersUserRecord.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsTypePapersUserRecord as NewQuestionsTypePapersUserRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsTypePapersUserRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsTypePapersUserRecord as NewQuestionsTypePapersUserRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsTypePapersUserRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsTypePapersUserRecord as NewQuestionsTypePapersUserRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
