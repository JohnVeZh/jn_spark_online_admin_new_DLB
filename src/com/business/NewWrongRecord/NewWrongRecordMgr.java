...
import com.business.NewWrongRecord.NewWrongRecord;
import com.business.NewWrongRecord.NewWrongRecordActionForm;


public class NewWrongRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewWrongRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewWrongRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewWrongRecord po = new NewWrongRecord();
	...
	*更新
	*/
	public void update(NewWrongRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewWrongRecord po = (NewWrongRecord) session.load(NewWrongRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewWrongRecord po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewWrongRecord) ses.load(NewWrongRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getNewWrongRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewWrongRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewWrongRecord po = (NewWrongRecord) ses.load(NewWrongRecord.class, ID);
			NewWrongRecordActionForm form = new NewWrongRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewWrongRecord as NewWrongRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewWrongRecord.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewWrongRecord as NewWrongRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewWrongRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewWrongRecord as NewWrongRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewWrongRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewWrongRecord as NewWrongRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
