...
import com.business.NetworkVideoEvaluate.NetworkVideoEvaluate;
import com.business.NetworkVideoEvaluate.NetworkVideoEvaluateActionForm;


public class NetworkVideoEvaluateMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NetworkVideoEvaluate.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NetworkVideoEvaluateActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NetworkVideoEvaluate po = new NetworkVideoEvaluate();
	...
	*更新
	*/
	public void update(NetworkVideoEvaluateActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NetworkVideoEvaluate po = (NetworkVideoEvaluate) session.load(NetworkVideoEvaluate.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NetworkVideoEvaluate po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NetworkVideoEvaluate) ses.load(NetworkVideoEvaluate.class, IDs[i]);
				//List list = ses.createFilter(po.getNetworkVideoEvaluate(), "").list();
	 ...
	 * @throws Exception
	 */
	public NetworkVideoEvaluateActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NetworkVideoEvaluate po = (NetworkVideoEvaluate) ses.load(NetworkVideoEvaluate.class, ID);
			NetworkVideoEvaluateActionForm form = new NetworkVideoEvaluateActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NetworkVideoEvaluate as NetworkVideoEvaluate where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NetworkVideoEvaluate.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoEvaluate as NetworkVideoEvaluate where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NetworkVideoEvaluate> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NetworkVideoEvaluate as NetworkVideoEvaluate where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NetworkVideoEvaluate.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoEvaluate as NetworkVideoEvaluate where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
