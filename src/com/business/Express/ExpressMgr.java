...
public class ExpressMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(Express.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ExpressActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			Express po = new Express();
	...
	*更新
	*/
	public void update(ExpressActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			Express po = (Express) session.load(Express.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (Express) ses.load(Express.class, IDs[i]);
				//List list = ses.createFilter(po.getExpress(), "").list();
	 ...
	 * @throws Exception
	 */
	public ExpressActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			Express po = (Express) ses.load(Express.class, ID);
			ExpressActionForm form = new ExpressActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from Express as Express where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by Express.id   ";
			Query qCount = ses
					.createQuery("select count(id) from Express as Express where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<Express> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from Express as Express where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by Express.id desc";
			Query qCount = ses
					.createQuery("select count(id) from Express as Express where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
