...
public class ProductOrderEvaluateMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductOrderEvaluate.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductOrderEvaluateActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductOrderEvaluate po = new ProductOrderEvaluate();
	...
	*更新
	*/
	public void update(ProductOrderEvaluateActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductOrderEvaluate po = (ProductOrderEvaluate) session.load(ProductOrderEvaluate.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			ProductOrderEvaluate po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductOrderEvaluate) ses.load(ProductOrderEvaluate.class, IDs[i]);
				//List list = ses.createFilter(po.getProductOrderEvaluate(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductOrderEvaluateActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductOrderEvaluate po = (ProductOrderEvaluate) ses.load(ProductOrderEvaluate.class, ID);
			ProductOrderEvaluateActionForm form = new ProductOrderEvaluateActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductOrderEvaluate as ProductOrderEvaluate where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductOrderEvaluate.createtime desc   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderEvaluate as ProductOrderEvaluate where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductOrderEvaluate> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductOrderEvaluate as ProductOrderEvaluate where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductOrderEvaluate.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderEvaluate as ProductOrderEvaluate where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
