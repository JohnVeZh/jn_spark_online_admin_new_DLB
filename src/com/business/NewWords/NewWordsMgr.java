...
import com.business.NewWords.NewWords;
import com.business.NewWords.NewWordsActionForm;


public class NewWordsMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewWords.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewWordsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewWords po = new NewWords();
	...
	*更新
	*/
	public void update(NewWordsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewWords po = (NewWords) session.load(NewWords.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NewWords) ses.load(NewWords.class, IDs[i]);
				//List list = ses.createFilter(po.getNewWords(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewWordsActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewWords po = (NewWords) ses.load(NewWords.class, ID);
			NewWordsActionForm form = new NewWordsActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewWords as NewWords where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewWords.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewWords as NewWords where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewWords> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewWords as NewWords where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewWords.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewWords as NewWords where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
