...
public class JQrMpthsMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrMpths.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrMpthsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(JQrMpthsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrMpths po = (JQrMpths) session.load(JQrMpths.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrMpths) ses.load(JQrMpths.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrMpths(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrMpthsActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrMpths po = (JQrMpths) ses.load(JQrMpths.class, ID);
			JQrMpthsActionForm form = new JQrMpthsActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrMpths as JQrMpths where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrMpths.sNumber ";
			Query qCount = ses
					.createQuery("select count(id) from JQrMpths as JQrMpths where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrMpths> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrMpths as JQrMpths where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrMpths.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrMpths as JQrMpths where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
