...
import com.business.PreferentialCode.PreferentialCode;
 ...
 */
public class TbCouponMgr extends AbstractHibernateDAO {

    private static LogUtils logger = LogUtils.getLogger(PreferentialCode.class);
     ...
     *           要抛出的异常 可定义多个
     */
    public void add(TbCouponActionForm tb) throws SystemException, Exception {
        Transaction tx = null;

        try {
            TbCoupon po = new TbCoupon();
            Session ses = HibernateSessionFactory.openSession();
            tx = ses.beginTransaction();
            super.copyProperties(po, tb);
            ses.save(po);
            tx.commit();
        } catch (Exception e) {
            e.printStackTrace();
     ...
     *更新
     */
    public void update(TbCouponActionForm vo) throws SystemException, Exception {
        Transaction tx = null;
        try {

            Session session = HibernateSessionFactory.openSession();
            tx = session.beginTransaction();
            TbCoupon po = (TbCoupon) session.load(TbCoupon.class, vo.getId());
    ...
    public void delete(String[] IDs) throws SystemException, Exception {
        ...
        try {
            ...
            for (int i = 0; i < IDs.length; i++) {
                po = (PreferentialCode) ses.load(PreferentialCode.class, IDs[i]);
                //List list = ses.createFilter(po.getPreferentialCode(), "").list();
     ...
     * @throws Exception
     */
    public TbCouponActionForm view(String ID) throws SystemException, Exception {
        try {
            Session ses = HibernateSessionFactory.openSession();
            TbCoupon tb = (TbCoupon) ses.load(TbCoupon.class, ID);
            TbCouponActionForm form = new TbCouponActionForm();
            super.copyProperties(form, tb);
            return form;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error(e);
    ...
    public ListContainer list(Collection queryConds, int currentPageInt,
        ...
        try {
            ...
            Session ses = HibernateSessionFactory.openSession();
            // 查询对象
            String hq = "from TbCoupon where 1=1"+ QueryHelper.toAndQuery(queryConds)+ " order by endTime desc,createTime desc";
            Query qCount = ses
                    .createQuery("select count(id) from TbCoupon where 1=1 "
     ...
     */
    public List<PreferentialCode> mlist(Collection queryConds,MobilePage mp) throws SystemException,
            Exception {
        try {
            // 在数据准备完成后，获取hiernate会话
            Session ses = HibernateSessionFactory.openSession();
            ...
            // 保存对象
            String hq = "from PreferentialCode as PreferentialCode where 1=1 "
                    + QueryHelper.toAndQuery(queryConds) + " order by PreferentialCode.id desc";
            Query qCount = ses
                    .createQuery("select count(id) from PreferentialCode as PreferentialCode where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
