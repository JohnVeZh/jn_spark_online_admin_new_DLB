...
import com.business.JQrWritingType.JQrWritingTypeActionForm;


public class JQrWritingTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrWritingType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrWritingTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrWritingType po = new JQrWritingType();
	...
	*更新
	*/
	public void update(JQrWritingTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrWritingType po = (JQrWritingType) session.load(JQrWritingType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			JQrWritingType po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrWritingType) ses.load(JQrWritingType.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrWritingType(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrWritingTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrWritingType po = (JQrWritingType) ses.load(JQrWritingType.class, ID);
			JQrWritingTypeActionForm form = new JQrWritingTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrWritingType as JQrWritingType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrWritingType.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrWritingType as JQrWritingType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrWritingType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrWritingType as JQrWritingType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrWritingType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrWritingType as JQrWritingType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
