...
public class MatchedPapersTopicLyricMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicLyric.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicLyricActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicLyric po = new MatchedPapersTopicLyric();
	...
	*更新
	*/
	public void update(MatchedPapersTopicLyricActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicLyric po = (MatchedPapersTopicLyric) session.load(MatchedPapersTopicLyric.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicLyric) ses.load(MatchedPapersTopicLyric.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicLyric(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicLyricActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicLyric po = (MatchedPapersTopicLyric) ses.load(MatchedPapersTopicLyric.class, ID);
			MatchedPapersTopicLyricActionForm form = new MatchedPapersTopicLyricActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicLyric as MatchedPapersTopicLyric where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicLyric.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicLyric as MatchedPapersTopicLyric where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicLyric> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicLyric as MatchedPapersTopicLyric where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicLyric.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicLyric as MatchedPapersTopicLyric where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
