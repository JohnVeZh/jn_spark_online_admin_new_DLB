...
public class PushLogMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(PushLog.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(PushLogActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			PushLog po = new PushLog();
	...
	*更新
	*/
	public void update(PushLogActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			PushLog po = (PushLog) session.load(PushLog.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (PushLog) ses.load(PushLog.class, IDs[i]);
				//List list = ses.createFilter(po.getPushLog(), "").list();
	 ...
	 * @throws Exception
	 */
	public PushLogActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			PushLog po = (PushLog) ses.load(PushLog.class, ID);
			PushLogActionForm form = new PushLogActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from PushLog as PushLog where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by PushLog.createtime desc   ";
			Query qCount = ses
					.createQuery("select count(id) from PushLog as PushLog where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<PushLog> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from PushLog as PushLog where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by PushLog.id desc";
			Query qCount = ses
					.createQuery("select count(id) from PushLog as PushLog where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
