...
public class NewsMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(News.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(NewsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			News po = (News) session.load(News.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (News) ses.load(News.class, IDs[i]);
				//List list = ses.createFilter(po.getNews(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewsActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			News po = (News) ses.load(News.class, ID);
			NewsActionForm form = new NewsActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from News as News where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by News.ptime desc,News.topTime desc";
			Query qCount = ses
					.createQuery("select count(id) from News as News where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<News> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from News as News where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by News.ptime desc";
			Query qCount = ses
					.createQuery("select count(id) from News as News where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
