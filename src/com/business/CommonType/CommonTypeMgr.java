...
import com.business.CommonType.CommonTypeActionForm;
import com.business.ProductType.ProductType;


public class CommonTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(CommonType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(CommonTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			CommonType po = new CommonType();
	...
	*更新
	*/
	public void update(CommonTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			CommonType po = (CommonType) session.load(CommonType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (CommonType) ses.load(CommonType.class, IDs[i]);
				//List list = ses.createFilter(po.getCommonType(), "").list();
	 ...
	 * @throws Exception
	 */
	public CommonTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			CommonType po = (CommonType) ses.load(CommonType.class, ID);
			CommonTypeActionForm form = new CommonTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from CommonType as CommonType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by CommonType.id   ";
			Query qCount = ses
					.createQuery("select count(id) from CommonType as CommonType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<CommonType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from CommonType as CommonType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by CommonType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from CommonType as CommonType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
	...
	public List getFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			while (tempArray.hasNext()) {
				...
				treeNode
						.setTreeNodeURL("CommonType.do?act=list&parentid="
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List getFuntreeList(String parentId) throws SystemException, Exception {
		List list = null;
		List treeNodes = new ArrayList();
		try {
			Collection queryConds = new ArrayList();
