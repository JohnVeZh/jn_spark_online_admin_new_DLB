...
public class ProductOrderDetailsCollocationMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductOrderDetailsCollocation.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductOrderDetailsCollocationActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductOrderDetailsCollocation po = new ProductOrderDetailsCollocation();
	...
	*更新
	*/
	public void update(ProductOrderDetailsCollocationActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductOrderDetailsCollocation po = (ProductOrderDetailsCollocation) session.load(ProductOrderDetailsCollocation.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			ProductOrderDetailsCollocation po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductOrderDetailsCollocation) ses.load(ProductOrderDetailsCollocation.class, IDs[i]);
				//List list = ses.createFilter(po.getProductOrderDetailsCollocation(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductOrderDetailsCollocationActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductOrderDetailsCollocation po = (ProductOrderDetailsCollocation) ses.load(ProductOrderDetailsCollocation.class, ID);
			ProductOrderDetailsCollocationActionForm form = new ProductOrderDetailsCollocationActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductOrderDetailsCollocation as ProductOrderDetailsCollocation where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductOrderDetailsCollocation.id   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderDetailsCollocation as ProductOrderDetailsCollocation where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductOrderDetailsCollocation> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductOrderDetailsCollocation as ProductOrderDetailsCollocation where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductOrderDetailsCollocation.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderDetailsCollocation as ProductOrderDetailsCollocation where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
