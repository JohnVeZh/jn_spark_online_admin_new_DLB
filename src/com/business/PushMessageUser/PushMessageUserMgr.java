...
import com.business.PushMessageUser.PushMessageUser;
import com.business.PushMessageUser.PushMessageUserActionForm;


public class PushMessageUserMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(PushMessageUser.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(PushMessageUserActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			PushMessageUser po = new PushMessageUser();
	...
	*更新
	*/
	public void update(PushMessageUserActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			PushMessageUser po = (PushMessageUser) session.load(PushMessageUser.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			PushMessageUser po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (PushMessageUser) ses.load(PushMessageUser.class, IDs[i]);
				//List list = ses.createFilter(po.getPushMessageUser(), "").list();
	 ...
	 * @throws Exception
	 */
	public PushMessageUserActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			PushMessageUser po = (PushMessageUser) ses.load(PushMessageUser.class, ID);
			PushMessageUserActionForm form = new PushMessageUserActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from PushMessageUser as PushMessageUser where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by PushMessageUser.id   ";
			Query qCount = ses
					.createQuery("select count(id) from PushMessageUser as PushMessageUser where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<PushMessageUser> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from PushMessageUser as PushMessageUser where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by PushMessageUser.id desc";
			Query qCount = ses
					.createQuery("select count(id) from PushMessageUser as PushMessageUser where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
