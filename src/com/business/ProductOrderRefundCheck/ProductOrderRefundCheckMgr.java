...
import com.business.ProductOrderRefundCheck.ProductOrderRefundCheckActionForm;
...
public class ProductOrderRefundCheckMgr extends AbstractHibernateDAO {
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductOrderRefundCheckActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(ProductOrderRefundCheckActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
    ...
    public <T> List<T> getListBySql(String sql, int firstResult, int maxResult) {
	 ...
	 * @throws Exception
	 */
	public ProductOrderRefundCheckActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductOrderRefund po = (ProductOrderRefund) ses.load(ProductOrderRefund.class, ID);
			ProductOrderRefundCheckActionForm form = new ProductOrderRefundCheckActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt, int itemsInPage, String action, int jumpPage) throws SystemException, Exception {
		try {
			...
			// 查询对象
			String hq = "from ProductOrderRefund as ProductOrderRefund where 1=1 " + QueryHelper.toAndQuery(queryConds)
					+ " order by ProductOrderRefund.createtime  desc ";
			Query qCount = ses.createQuery("select count(id) from ProductOrderRefund as ProductOrderRefund where 1=1 " + QueryHelper.toAndQuery(queryConds));
