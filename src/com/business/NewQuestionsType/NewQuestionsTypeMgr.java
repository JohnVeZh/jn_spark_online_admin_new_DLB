...
public class NewQuestionsTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsType.class);
	...
	public List getFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			NewQuestionsType function = null;
			TreeNode treeNode = null;
			//查询英语级别
			List<SysConfig> lt = Tool.findListByHql("from SysConfig where type='MT_TYPE'");
			for (SysConfig sysConfig : lt) {
				
				treeNode = new TreeNode(sysConfig.getValue(), sysConfig.getName(),"");
				treeNode
				.setTreeNodeURL("NewQuestionsType.do?act=list&parentId="+sysConfig.getValue()+"&levelType="+sysConfig.getValue());//+"&state="+function.getState()
				treeNodes.add(treeNode);
			}
			
			Collection queryConds = new ArrayList();
			Session ses = HibernateSessionFactory.openSession();
			String queryString = "from NewQuestionsType";
	...
	public List getMPFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			for (SysConfig sysConfig : lt) {
				...
				treeNode = new TreeNode(sysConfig.getValue(), sysConfig.getName(),"");
				treeNode
				.setTreeNodeURL("NewQuestionsTypePapers.do?act=list&nqtId="
						+ sysConfig.getValue()+"&levelType="+sysConfig.getValue());//+"&state="+function.getState()
				treeNodes.add(treeNode);
			}
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsType po = new NewQuestionsType();
			Session ses = HibernateSessionFactory.openSession();
	...
	*更新
	*/
	public void update(NewQuestionsTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsType po = (NewQuestionsType) session.load(NewQuestionsType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewQuestionsType po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsType) ses.load(NewQuestionsType.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsType(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsType po = (NewQuestionsType) ses.load(NewQuestionsType.class, ID);
			NewQuestionsTypeActionForm form = new NewQuestionsTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsType as NewQuestionsType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsType.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsType as NewQuestionsType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsType as NewQuestionsType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsType as NewQuestionsType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
