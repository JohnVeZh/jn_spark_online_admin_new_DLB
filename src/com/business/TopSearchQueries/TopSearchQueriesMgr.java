...
import com.business.TopSearchQueries.TopSearchQueries;
import com.business.TopSearchQueries.TopSearchQueriesActionForm;


public class TopSearchQueriesMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(TopSearchQueries.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(TopSearchQueriesActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			TopSearchQueries po = new TopSearchQueries();
	...
	*更新
	*/
	public void update(TopSearchQueriesActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			TopSearchQueries po = (TopSearchQueries) session.load(TopSearchQueries.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (TopSearchQueries) ses.load(TopSearchQueries.class, IDs[i]);
				//List list = ses.createFilter(po.getTopSearchQueries(), "").list();
	 ...
	 * @throws Exception
	 */
	public TopSearchQueriesActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			TopSearchQueries po = (TopSearchQueries) ses.load(TopSearchQueries.class, ID);
			TopSearchQueriesActionForm form = new TopSearchQueriesActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from TopSearchQueries as TopSearchQueries where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by TopSearchQueries.id   ";
			Query qCount = ses
					.createQuery("select count(id) from TopSearchQueries as TopSearchQueries where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<TopSearchQueries> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from TopSearchQueries as TopSearchQueries where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by TopSearchQueries.id desc";
			Query qCount = ses
					.createQuery("select count(id) from TopSearchQueries as TopSearchQueries where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
