...
public class PushMessageMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(PushMessage.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(PushMessageActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			PushMessage po = new PushMessage();
	...
	*更新
	*/
	public void update(PushMessageActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			PushMessage po = (PushMessage) session.load(PushMessage.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (PushMessage) ses.load(PushMessage.class, IDs[i]);
				//List list = ses.createFilter(po.getPushMessage(), "").list();
	 ...
	 * @throws Exception
	 */
	public PushMessageActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			PushMessage po = (PushMessage) ses.load(PushMessage.class, ID);
			PushMessageActionForm form = new PushMessageActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from PushMessage as PushMessage where type=0  "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by PushMessage.pmCreatetime desc";
			Query qCount = ses
					.createQuery("select count(id) from PushMessage as PushMessage where 1=1 and type=0   "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<PushMessage> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from PushMessage as PushMessage where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by PushMessage.id desc";
			Query qCount = ses
					.createQuery("select count(id) from PushMessage as PushMessage where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
