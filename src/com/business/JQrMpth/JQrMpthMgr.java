...
import com.business.JQrMpth.JQrMpthActionForm;


public class JQrMpthMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrMpth.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrMpthActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrMpth po = new JQrMpth();
	...
	*更新
	*/
	public void update(JQrMpthActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrMpth po = (JQrMpth) session.load(JQrMpth.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrMpth) ses.load(JQrMpth.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrMpth(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrMpthActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrMpth po = (JQrMpth) ses.load(JQrMpth.class, ID);
			JQrMpthActionForm form = new JQrMpthActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrMpth as JQrMpth where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrMpth.sort ";
			Query qCount = ses
					.createQuery("select count(id) from JQrMpth as JQrMpth where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrMpth> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrMpth as JQrMpth where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrMpth.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrMpth as JQrMpth where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
