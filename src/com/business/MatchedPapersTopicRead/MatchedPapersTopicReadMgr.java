...
public class MatchedPapersTopicReadMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicRead.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicReadActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicRead po = new MatchedPapersTopicRead();
	...
	*更新
	*/
	public void update(MatchedPapersTopicReadActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicRead po = (MatchedPapersTopicRead) session.load(MatchedPapersTopicRead.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			MatchedPapersTopicRead po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicRead) ses.load(MatchedPapersTopicRead.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicRead(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicReadActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicRead po = (MatchedPapersTopicRead) ses.load(MatchedPapersTopicRead.class, ID);
			MatchedPapersTopicReadActionForm form = new MatchedPapersTopicReadActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicRead as MatchedPapersTopicRead where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicRead.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicRead as MatchedPapersTopicRead where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicRead> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicRead as MatchedPapersTopicRead where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicRead.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicRead as MatchedPapersTopicRead where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
