...
import com.business.TbCouponRecord.TbCouponRecord;
 ...
 */
public class TbCouponRecordMgr extends AbstractHibernateDAO {
    private static LogUtils logger = LogUtils.getLogger(TbCouponRecord.class);
     ...
     *           要抛出的异常 可定义多个
     */
    public void add(TbCouponRecordActionForm vo) throws SystemException, Exception {
        Transaction tx = null;

        try {
            TbCouponRecord po = new TbCouponRecord();
     ...
     *更新
     */
    public void update(TbCouponRecordActionForm vo) throws SystemException, Exception {
        Transaction tx = null;
        try {

            Session session = HibernateSessionFactory.openSession();
            tx = session.beginTransaction();
            TbCouponRecord po = (TbCouponRecord) session.load(TbCouponRecord.class, vo.getId());
            super.copyProperties(po, vo);
            session.update(po);
            tx.commit();
        } catch (Exception e) {
    ...
    public void delete(String[] IDs) throws SystemException, Exception {
        ...
        try {
            ...
            for (int i = 0; i < IDs.length; i++) {
                po = (TbCouponRecord) ses.load(TbCouponRecord.class, IDs[i]);
                //List list = ses.createFilter(po.getTbCouponRecord(), "").list();
     ...
     * @throws Exception
     */
    public TbCouponRecordActionForm view(String ID) throws SystemException, Exception {
        try {
            Session ses = HibernateSessionFactory.openSession();
            TbCouponRecord po = (TbCouponRecord) ses.load(TbCouponRecord.class, ID);
            TbCouponRecordActionForm form = new TbCouponRecordActionForm();
    ...
    public ListContainer list(Collection queryConds, int currentPageInt,
        ...
        try {
            ...
            // 查询对象
            String hq = "from TbCouponRecord as TbCouponRecord where 1=1 "
                    + QueryHelper.toAndQuery(queryConds)
                    + " order by TbCouponRecord.useTime desc  ";
            Query qCount = ses
                    .createQuery("select count(id) from TbCouponRecord as TbCouponRecord where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
     ...
     */
    public List<TbCouponRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
            Exception {
        try {
            // 在数据准备完成后，获取hiernate会话
            Session ses = HibernateSessionFactory.openSession();
            ...
            // 保存对象
            String hq = "from TbCouponRecord as TbCouponRecord where 1=1 "
                    + QueryHelper.toAndQuery(queryConds) + " order by TbCouponRecord.id desc";
            Query qCount = ses
                    .createQuery("select count(id) from TbCouponRecord as TbCouponRecord where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
