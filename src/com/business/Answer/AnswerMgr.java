...
public class AnswerMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(Answer.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(AnswerActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(AnswerActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			Answer po = (Answer) session.load(Answer.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (Answer) ses.load(Answer.class, IDs[i]);
				//List list = ses.createFilter(po.getAnswer(), "").list();
	 ...
	 * @throws Exception
	 */
	public AnswerActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			Answer po = (Answer) ses.load(Answer.class, ID);
			AnswerActionForm form = new AnswerActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from Answer as Answer where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by sort   ";
			Query qCount = ses
					.createQuery("select count(id) from Answer as Answer where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<Answer> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from Answer as Answer where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by Answer.id desc";
			Query qCount = ses
					.createQuery("select count(id) from Answer as Answer where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
