...
import com.business.ProductNetworkVideo.ProductNetworkVideoActionForm;


public class ProductNetworkVideoMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductNetworkVideo.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductNetworkVideoActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductNetworkVideo po = new ProductNetworkVideo();
	...
	*更新
	*/
	public void update(ProductNetworkVideoActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductNetworkVideo po = (ProductNetworkVideo) session.load(ProductNetworkVideo.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductNetworkVideo) ses.load(ProductNetworkVideo.class, IDs[i]);
				//List list = ses.createFilter(po.getProductNetworkVideo(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductNetworkVideoActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductNetworkVideo po = (ProductNetworkVideo) ses.load(ProductNetworkVideo.class, ID);
			ProductNetworkVideoActionForm form = new ProductNetworkVideoActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductNetworkVideo as ProductNetworkVideo where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductNetworkVideo.id   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductNetworkVideo as ProductNetworkVideo where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductNetworkVideo> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductNetworkVideo as ProductNetworkVideo where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductNetworkVideo.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductNetworkVideo as ProductNetworkVideo where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
