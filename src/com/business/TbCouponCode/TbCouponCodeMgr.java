...
import com.business.TbCouponCode.TbCouponCode;
import com.business.TbCouponCode.TbCouponCodeActionForm;
 ...
 */
public class TbCouponCodeMgr extends AbstractHibernateDAO {
    private static LogUtils logger = LogUtils.getLogger(TbCouponCode.class);
     ...
     *           要抛出的异常 可定义多个
     */
    public void add(TbCouponCodeActionForm vo) throws SystemException, Exception {
        Transaction tx = null;

        try {
            TbCouponCode po = new TbCouponCode();
     ...
     *更新
     */
    public void update(TbCouponCodeActionForm vo) throws SystemException, Exception {
        Transaction tx = null;
        try {

            Session session = HibernateSessionFactory.openSession();
            tx = session.beginTransaction();
            TbCouponCode po = (TbCouponCode) session.load(TbCouponCode.class, vo.getId());
            super.copyProperties(po, vo);
            session.update(po);
            tx.commit();
        } catch (Exception e) {
    ...
    public void delete(String[] IDs) throws SystemException, Exception {
        ...
        try {
            ...
            for (int i = 0; i < IDs.length; i++) {
                po = (TbCouponCode) ses.load(TbCouponCode.class, IDs[i]);
                //List list = ses.createFilter(po.getTbCouponCode(), "").list();
     ...
     * @throws Exception
     */
    public TbCouponCodeActionForm view(String ID) throws SystemException, Exception {
        try {
            Session ses = HibernateSessionFactory.openSession();
            TbCouponCode po = (TbCouponCode) ses.load(TbCouponCode.class, ID);
            TbCouponCodeActionForm form = new TbCouponCodeActionForm();
    ...
    public ListContainer list(Collection queryConds, int currentPageInt,
        ...
        try {
            ...
            // 查询对象
            String hq = "from TbCouponCode as TbCouponCode where 1=1 "
                    + QueryHelper.toAndQuery(queryConds)
                    + " order by TbCouponCode.id   ";
            Query qCount = ses
                    .createQuery("select count(id) from TbCouponCode as TbCouponCode where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
     ...
     */
    public List<TbCouponCode> mlist(Collection queryConds,MobilePage mp) throws SystemException,
            Exception {
        try {
            // 在数据准备完成后，获取hiernate会话
            Session ses = HibernateSessionFactory.openSession();
            ...
            // 保存对象
            String hq = "from TbCouponCode as TbCouponCode where 1=1 "
                    + QueryHelper.toAndQuery(queryConds) + " order by TbCouponCode.id desc";
            Query qCount = ses
                    .createQuery("select count(id) from TbCouponCode as TbCouponCode where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
