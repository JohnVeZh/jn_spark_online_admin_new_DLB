...
import com.business.MatchedPapersTopicReadType.MatchedPapersTopicReadTypeActionForm;


public class MatchedPapersTopicReadTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicReadType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicReadTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicReadType po = new MatchedPapersTopicReadType();
	...
	*更新
	*/
	public void update(MatchedPapersTopicReadTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicReadType po = (MatchedPapersTopicReadType) session.load(MatchedPapersTopicReadType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicReadType) ses.load(MatchedPapersTopicReadType.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicReadType(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicReadTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicReadType po = (MatchedPapersTopicReadType) ses.load(MatchedPapersTopicReadType.class, ID);
			MatchedPapersTopicReadTypeActionForm form = new MatchedPapersTopicReadTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicReadType as MatchedPapersTopicReadType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicReadType.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicReadType as MatchedPapersTopicReadType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicReadType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicReadType as MatchedPapersTopicReadType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicReadType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicReadType as MatchedPapersTopicReadType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
