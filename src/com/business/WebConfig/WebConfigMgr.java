...
import com.business.WebConfig.WebConfig;
import com.business.WebConfig.WebConfigActionForm;


public class WebConfigMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(WebConfig.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(WebConfigActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			WebConfig po = new WebConfig();
	...
	*更新
	*/
	public void update(WebConfigActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			WebConfig po = (WebConfig) session.load(WebConfig.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (WebConfig) ses.load(WebConfig.class, IDs[i]);
				//List list = ses.createFilter(po.getWebConfig(), "").list();
	 ...
	 * @throws Exception
	 */
	public WebConfigActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			WebConfig po = (WebConfig) ses.load(WebConfig.class, ID);
			WebConfigActionForm form = new WebConfigActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from WebConfig as WebConfig where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by WebConfig.id   ";
			Query qCount = ses
					.createQuery("select count(id) from WebConfig as WebConfig where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<WebConfig> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from WebConfig as WebConfig where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by WebConfig.id desc";
			Query qCount = ses
					.createQuery("select count(id) from WebConfig as WebConfig where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
