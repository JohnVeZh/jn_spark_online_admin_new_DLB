...
public class JCommonWordsMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JCommonWords.class);
	...
	public List getMPFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			JCommonType function = null;
			TreeNode treeNode = null;
			
			Collection queryConds = new ArrayList();
			Session ses = HibernateSessionFactory.openSession();
			String queryString = "from JCommonType ";
			...
			while (tempArray.hasNext()) {
				function =  (JCommonType)tempArray.next();
				treeNode = new TreeNode(function.getId(), function.getTypeName(), null);
				treeNode
						.setTreeNodeURL("JCommonWords.do?act=list&ctId="
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JCommonWordsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JCommonWords po = new JCommonWords();
			Session ses = HibernateSessionFactory.openSession();
	...
	*更新
	*/
	public void update(JCommonWordsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JCommonWords po = (JCommonWords) session.load(JCommonWords.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			JCommonWords po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (JCommonWords) ses.load(JCommonWords.class, IDs[i]);
				//List list = ses.createFilter(po.getJCommonWords(), "").list();
	 ...
	 * @throws Exception
	 */
	public JCommonWordsActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JCommonWords po = (JCommonWords) ses.load(JCommonWords.class, ID);
			JCommonWordsActionForm form = new JCommonWordsActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JCommonWords as JCommonWords where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JCommonWords.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JCommonWords as JCommonWords where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JCommonWords> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JCommonWords as JCommonWords where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JCommonWords.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JCommonWords as JCommonWords where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
