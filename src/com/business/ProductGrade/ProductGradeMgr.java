...
import com.business.ProductGrade.ProductGrade;
import com.business.ProductGrade.ProductGradeActionForm;


public class ProductGradeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductGrade.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductGradeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductGrade po = new ProductGrade();
	...
	*更新
	*/
	public void update(ProductGradeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductGrade po = (ProductGrade) session.load(ProductGrade.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductGrade) ses.load(ProductGrade.class, IDs[i]);
				//List list = ses.createFilter(po.getProductGrade(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductGradeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductGrade po = (ProductGrade) ses.load(ProductGrade.class, ID);
			ProductGradeActionForm form = new ProductGradeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductGrade as ProductGrade where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductGrade.id   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductGrade as ProductGrade where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductGrade> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductGrade as ProductGrade where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductGrade.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductGrade as ProductGrade where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
