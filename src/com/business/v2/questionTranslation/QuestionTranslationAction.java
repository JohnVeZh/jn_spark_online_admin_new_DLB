...
import com.business.v2.questionWriting.TbQuestionWritingActionForm;
...
public class QuestionTranslationAction extends BaseAction{
	private static LogUtils logger = LogUtils.getLogger(TbQuestionTranslation.class);
	QuestionTranslationMgr mgr=new QuestionTranslationMgr();

	/**
	 *  绑定作业时需要的翻译列表
	 */
	public ActionForward questionTranslationList (ActionMapping mapping, ActionForm form,
		...
		try {
			...
			ListContainer lc = mgr.questionTranslationList(queryConds, currentPageInt,
					itemsInPage, action, jumpPage);
			request.setAttribute("title",title);
			request.setAttribute("lc", lc);
			return mapping.findForward("dlbTranslationList");
		} catch (Exception ex) {
	...
	public ActionForward realdeleteById(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			Tool.AddLog(sessionContainer.getUserId(), username, "专项练习翻译,翻译:"+name, "1", ipaddress);
	...
	public ActionForward preUpdate(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String sql="select a.name name,a.id catalogId,a.sort_order sortOrder,c.* from tb_special_catalog a LEFT JOIN tb_special_training b on a.id=b.catalog_id LEFT JOIN tb_question_translation c on b.training_id =c.id where a.id='"+ID+"' and a.is_del=0";
			Query query = ses.createSQLQuery(sql.toString()).addEntity(TbQuestionTranslation.class);
			if(query.list().size()>0){
				request.setAttribute("TbQuestionTranslation", query.list().get(0)); 
			}
	...
	public ActionForward update(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		TbQuestionTranslationActionForm vo = (TbQuestionTranslationActionForm) form;
		try {
			...
			String sql="UPDATE tb_question_translation SET title ='"+vo.getTitle()+"',content="+content+",reference="+reference+",analysis="+analysis+",target="+target+" WHERE id ='"+vo.getId()+"'";
			...
			String username = "admin";
			Tool.AddLog(sessionContainer.getUserId(), username, "修改翻译,翻译:"+vo.getName(), "1", ipaddress);
	...
	public ActionForward add(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//目录名称
			/*String max_name="TOPIC "+max_sort_order+" ｜ "+vo.getName();*/
			//级别 01:四级 02:六级
			...
			String trainingSQL="INSERT INTO tb_special_training(id,section,catalog_id,training_type,training_id,sort_order)VALUES"
					 + "('"+UUID.randomUUID().toString().replace("-", "")+"','"+p_id+"','"+max_id+"','3','"+vo.getId()+"','1')";
			//添加 tb_question_translation 翻译题表信息
			String analysis="\""+vo.getAnalysis()+"\"";
			String reference="\""+vo.getReference()+"\"";
			String content="\""+vo.getContent()+"\"";
			String target="\""+vo.getTarget()+"\"";
			String translationSQL="INSERT INTO tb_question_translation(id,title,content,reference,analysis,target,analysis_url,target,is_del,create_time)VALUES"
			...
			String username = "admin";
			Tool.AddLog(sessionContainer.getUserId(), username, "添加翻译,翻译:"+vo.getName(), "1", ipaddress);
	...
	public void preDown(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		try {
            ...
            String fileName = "翻译.xls";
            String filePath = "/template/training/翻译.xls";
	 ...
	 * @throws Exception
	 */
	public ActionForward QuestionTranslationUpload(ActionMapping actionMapping,ActionForm actionForm, HttpServletRequest request,HttpServletResponse response) throws Exception {
		SessionContainer sessionContainer = (SessionContainer) request.getSession().getAttribute("SessionContainer");
		if (null == sessionContainer)
			sessionContainer = new SessionContainer();
		Map m = new HashMap();
