...
public class QuestionTranslationMgr extends AbstractHibernateDAO{
	...
	public List getFuntree(String sql) throws SystemException, Exception {
		...
		try {
			...
			String queryString = "(select * from tb_special_catalog where id='0104') union ALL (select * from tb_special_catalog where id='0204')";
			...
			while (tempArray.hasNext()) {
				...
				treeNode.setTreeNodeURL("QuestionTranslation.do?act=list&Pid="+function.getId());//+"&state="+function.getState()
	...
	public void UploadAdd(String Pid, TbQuestionTranslation vo) throws SystemException, SQLException {
	 ...
	 * @return
	 */
    public ListContainer questionTranslationList(Collection queryConds, int currentPageInt, int itemsInPage, String action, int jumpPage) throws SystemException {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();

			// 查询对象
			String sq = "select * from tb_question_translation where target = '2' and is_del = '0' " + QueryHelper.toAndQuery(queryConds) + "order by create_time desc";
			Query qCount = ses
					.createSQLQuery("select count(id) from tb_question_translation where target = '2' and is_del = '0' " + QueryHelper.toAndQuery(queryConds) + "order by create_time desc");
