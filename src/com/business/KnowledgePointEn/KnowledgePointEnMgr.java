...
public class KnowledgePointEnMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(KnowledgePointEn.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(KnowledgePointEnActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			KnowledgePointEn po = new KnowledgePointEn();
	...
	*更新
	*/
	public void update(KnowledgePointEnActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			KnowledgePointEn po = (KnowledgePointEn) session.load(KnowledgePointEn.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (KnowledgePointEn) ses.load(KnowledgePointEn.class, IDs[i]);
				//List list = ses.createFilter(po.getKnowledgePointEn(), "").list();
	 ...
	 * @throws Exception
	 */
	public KnowledgePointEnActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			KnowledgePointEn po = (KnowledgePointEn) ses.load(KnowledgePointEn.class, ID);
			KnowledgePointEnActionForm form = new KnowledgePointEnActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from KnowledgePointEn as KnowledgePointEn where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by KnowledgePointEn.kpeCreatetime desc   ";
			Query qCount = ses
					.createQuery("select count(id) from KnowledgePointEn as KnowledgePointEn where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<KnowledgePointEn> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from KnowledgePointEn as KnowledgePointEn where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by KnowledgePointEn.id desc";
			Query qCount = ses
					.createQuery("select count(id) from KnowledgePointEn as KnowledgePointEn where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
