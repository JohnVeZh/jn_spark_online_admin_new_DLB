...
public class BookActivationCodeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils
			.getLogger(BookActivationCode.class);
	 ...
	 *             要抛出的异常 可定义多个
	 */
	public void add(BookActivationCodeActionForm vo) throws SystemException,
			Exception {
		Transaction tx = null;

		try {
			BookActivationCode po = new BookActivationCode();
	 ...
	 * 更新
	 */
	public void update(BookActivationCodeActionForm vo) throws SystemException,
			Exception {
		Transaction tx = null;
		try {
			...
			BookActivationCode po = (BookActivationCode) session.load(
					BookActivationCode.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (BookActivationCode) ses.load(BookActivationCode.class,
						IDs[i]);
				// List list = ses.createFilter(po.getBookActivationCode(),
				// "").list();
				// if (list.size() < 1 && po.getSysRoleFunrights().size() < 1)//
				// 没有引用
				ses.delete(po);
				// else
				// throw new Exception("功能:" + po.getName() +
				// "已经使用或存在下级功能或操作，不能删除!");
			}
			tx.commit();
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	public BookActivationCodeActionForm view(String ID) throws SystemException,
			Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			BookActivationCode po = (BookActivationCode) ses.load(
					BookActivationCode.class, ID);
			BookActivationCodeActionForm form = new BookActivationCodeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from BookActivationCode as BookActivationCode where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by BookActivationCode.createtime desc ";
			Query qCount = ses
					.createQuery("select count(id) from BookActivationCode as BookActivationCode where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
	 ...
	 */
	public List<BookActivationCode> mlist(Collection queryConds, MobilePage mp)
			throws SystemException, Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			String hq = "from BookActivationCode as BookActivationCode where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by BookActivationCode.id desc";
			Query qCount = ses
					.createQuery("select count(id) from BookActivationCode as BookActivationCode where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
     ...
     * @throws Exception
     */
	public List<BookActivationCode> mlist(Collection queryConds, String BookActivationCodeHql)
			throws SystemException, Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from BookActivationCode as BookActivationCode where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + BookActivationCodeHql;
			Query query = ses.createQuery(hq.toString());
			return query.list();
		} catch (Exception e) {
			logger.debug(e);
