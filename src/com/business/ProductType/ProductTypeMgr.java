...
public class ProductTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(ProductTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductType po = (ProductType) session.load(ProductType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductType) ses.load(ProductType.class, IDs[i]);
				//List list = ses.createFilter(po.getProductType(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductType po = (ProductType) ses.load(ProductType.class, ID);
			ProductTypeActionForm form = new ProductTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
			int itemsInPage, String action, int jumpPage,String sort) throws SystemException,
			Exception {
		try {
			...
			String hq = null;
			if("1".equals(sort)){
				hq = "from ProductType as ProductType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductType.sort    ";
			}else if("2".equals(sort)){
				hq = "from ProductType as ProductType where 1=1 "
						+ QueryHelper.toAndQuery(queryConds)
						+ " order by ProductType.sort desc   ";
			}else{
				hq = "from ProductType as ProductType where 1=1 "
						+ QueryHelper.toAndQuery(queryConds)
			...
			Query qCount = ses
					.createQuery("select count(id) from ProductType as ProductType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductType as ProductType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductType as ProductType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
