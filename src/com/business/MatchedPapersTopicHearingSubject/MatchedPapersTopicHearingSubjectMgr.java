...
public class MatchedPapersTopicHearingSubjectMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicHearingSubject.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicHearingSubjectActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicHearingSubject po = new MatchedPapersTopicHearingSubject();
	...
	*更新
	*/
	public void update(MatchedPapersTopicHearingSubjectActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicHearingSubject po = (MatchedPapersTopicHearingSubject) session.load(MatchedPapersTopicHearingSubject.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicHearingSubject) ses.load(MatchedPapersTopicHearingSubject.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicHearingSubject(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicHearingSubjectActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicHearingSubject po = (MatchedPapersTopicHearingSubject) ses.load(MatchedPapersTopicHearingSubject.class, ID);
			MatchedPapersTopicHearingSubjectActionForm form = new MatchedPapersTopicHearingSubjectActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicHearingSubject as MatchedPapersTopicHearingSubject where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicHearingSubject.sNumber   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearingSubject as MatchedPapersTopicHearingSubject where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicHearingSubject> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicHearingSubject as MatchedPapersTopicHearingSubject where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicHearingSubject.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearingSubject as MatchedPapersTopicHearingSubject where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
