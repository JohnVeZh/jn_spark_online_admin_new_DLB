...
public class CommunityPostMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(CommunityPost.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(CommunityPostActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(CommunityPostActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			CommunityPost po = (CommunityPost) session.load(CommunityPost.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (CommunityPost) ses.load(CommunityPost.class, IDs[i]);
				//List list = ses.createFilter(po.getCommunityPost(), "").list();
	 ...
	 * @throws Exception
	 */
	public CommunityPostActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			CommunityPost po = (CommunityPost) ses.load(CommunityPost.class, ID);
			CommunityPostActionForm form = new CommunityPostActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from CommunityPost as CommunityPost where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by CommunityPost.topTime desc,CommunityPost.createtime desc   ";
			Query qCount = ses
					.createQuery("select count(id) from CommunityPost as CommunityPost where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<CommunityPost> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from CommunityPost as CommunityPost where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by CommunityPost.id desc";
			Query qCount = ses
					.createQuery("select count(id) from CommunityPost as CommunityPost where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
