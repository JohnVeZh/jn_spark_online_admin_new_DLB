...
public class MatchedPapersUserRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersUserRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersUserRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersUserRecord po = new MatchedPapersUserRecord();
	...
	*更新
	*/
	public void update(MatchedPapersUserRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersUserRecord po = (MatchedPapersUserRecord) session.load(MatchedPapersUserRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			MatchedPapersUserRecord po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersUserRecord) ses.load(MatchedPapersUserRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersUserRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersUserRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersUserRecord po = (MatchedPapersUserRecord) ses.load(MatchedPapersUserRecord.class, ID);
			MatchedPapersUserRecordActionForm form = new MatchedPapersUserRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersUserRecord as MatchedPapersUserRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersUserRecord.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersUserRecord as MatchedPapersUserRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersUserRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersUserRecord as MatchedPapersUserRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersUserRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersUserRecord as MatchedPapersUserRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
