...
public class PeriodVideoDetailActionMgr extends AbstractHibernateDAO{
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq="SELECT a.*,b.username username FROM dlb_period_video_detail a LEFT JOIN users b ON a.create_by=b.id WHERE 1=1 AND a.del_flag!=1  "+ QueryHelper.toAndQuery(queryConds)+"  ORDER BY a.sort asc  ";
			
			Query qCount = ses .createSQLQuery("SELECT count(1) FROM dlb_period_video_detail a LEFT JOIN users b ON a.create_by=b.id WHERE 1=1 AND a.del_flag!=1  "+ QueryHelper.toAndQuery(queryConds));
			...
			Query query = ses.createSQLQuery(hq.toString()).addEntity(PeriodVideoDetail.class);
	...
	public static List findObjectListBySql (String sql) throws SystemException, Exception {
		try {
			...
			Session ses = HibernateSessionFactory.openSession();
			// 保存对象
			Query query = ses.createSQLQuery(sql.toString()).addEntity(PeriodVideoDetail.class);;
			return query.list();
