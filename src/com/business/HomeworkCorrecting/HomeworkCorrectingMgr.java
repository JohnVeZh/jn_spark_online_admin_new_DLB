...
public class HomeworkCorrectingMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(HomeworkCorrecting.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(HomeworkCorrectingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			HomeworkCorrecting po = new HomeworkCorrecting();
	...
	*更新
	*/
	public void update(HomeworkCorrectingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			HomeworkCorrecting po = (HomeworkCorrecting) session.load(HomeworkCorrecting.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (HomeworkCorrecting) ses.load(HomeworkCorrecting.class, IDs[i]);
				//List list = ses.createFilter(po.getHomeworkCorrecting(), "").list();
	 ...
	 * @throws Exception
	 */
	public HomeworkCorrectingActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			HomeworkCorrecting po = (HomeworkCorrecting) ses.load(HomeworkCorrecting.class, ID);
			HomeworkCorrectingActionForm form = new HomeworkCorrectingActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from HomeworkCorrecting as HomeworkCorrecting where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by HomeworkCorrecting.id   ";
			Query qCount = ses
					.createQuery("select count(id) from HomeworkCorrecting as HomeworkCorrecting where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<HomeworkCorrecting> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from HomeworkCorrecting as HomeworkCorrecting where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by HomeworkCorrecting.id desc";
			Query qCount = ses
					.createQuery("select count(id) from HomeworkCorrecting as HomeworkCorrecting where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
