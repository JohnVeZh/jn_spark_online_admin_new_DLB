...
public class CommunityPostMessageMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(CommunityPostMessage.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(CommunityPostMessageActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			CommunityPostMessage po = new CommunityPostMessage();
	...
	*更新
	*/
	public void update(CommunityPostMessageActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			CommunityPostMessage po = (CommunityPostMessage) session.load(CommunityPostMessage.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			CommunityPostMessage po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (CommunityPostMessage) ses.load(CommunityPostMessage.class, IDs[i]);
				//List list = ses.createFilter(po.getCommunityPostMessage(), "").list();
	 ...
	 * @throws Exception
	 */
	public CommunityPostMessageActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			CommunityPostMessage po = (CommunityPostMessage) ses.load(CommunityPostMessage.class, ID);
			CommunityPostMessageActionForm form = new CommunityPostMessageActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String sql = "SELECT cp.title, cpm.id, cpm.community_post_id, cpm.user_id, " +
					" cast(cpm.content as char(500)), cpm.createtime, cpm.comments_id, " +
					" cpm.like_num, cpm.imgpths, cpm.firstmes_id, cpm.is_top, " +
					" cpm.top_time, users.username, users.mobile FROM " +
					" community_post_message cpm " +
					" left JOIN community_post cp ON cpm.community_post_id = cp.id " +
					" left JOIN users ON cpm.user_id = users.id " +
					"where 1=1  "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by cpm.top_time desc, cpm.createtime desc";
			Query qCount = ses
...
/**
	 ...
	 */
	public List<CommunityPostMessage> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from CommunityPostMessage as CommunityPostMessage where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by CommunityPostMessage.id desc";
			Query qCount = ses
					.createQuery("select count(id) from CommunityPostMessage as CommunityPostMessage where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
