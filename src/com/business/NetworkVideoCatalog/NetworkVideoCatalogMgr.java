...
public class NetworkVideoCatalogMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NetworkVideoCatalog.class);
	...
	public List getFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			while (tempArray.hasNext()) {
				...
				treeNode
						.setTreeNodeURL("NetworkVideoCatalog.do?act=list&parentId="
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NetworkVideoCatalogActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NetworkVideoCatalog po = new NetworkVideoCatalog();
			Session ses = HibernateSessionFactory.openSession();
	...
	*更新
	*/
	public void update(NetworkVideoCatalogActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NetworkVideoCatalog po = (NetworkVideoCatalog) session.load(NetworkVideoCatalog.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NetworkVideoCatalog po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NetworkVideoCatalog) ses.load(NetworkVideoCatalog.class, IDs[i]);
				//List list = ses.createFilter(po.getNetworkVideoCatalog(), "").list();
	 ...
	 * @throws Exception
	 */
	public NetworkVideoCatalogActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NetworkVideoCatalog po = (NetworkVideoCatalog) ses.load(NetworkVideoCatalog.class, ID);
			NetworkVideoCatalogActionForm form = new NetworkVideoCatalogActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NetworkVideoCatalog as NetworkVideoCatalog where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NetworkVideoCatalog.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoCatalog as NetworkVideoCatalog where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NetworkVideoCatalog> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NetworkVideoCatalog as NetworkVideoCatalog where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NetworkVideoCatalog.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoCatalog as NetworkVideoCatalog where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
