...
public class MatchedPapersTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersType.class);
	...
	public List getFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			MatchedPapersType function = null;
			...
			for (SysConfig sysConfig : lt) {
				...
				treeNode
				.setTreeNodeURL("MatchedPapersType.do?act=list&parentId="+sysConfig.getValue()+"&scId="+sysConfig.getValue());//+"&state="+function.getState()
				treeNodes.add(treeNode);
			}
			
			Collection queryConds = new ArrayList();
			Session ses = HibernateSessionFactory.openSession();
			String queryString = "from MatchedPapersType";
	...
	public List getMPFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			for (SysConfig sysConfig : lt) {
				...
				treeNode = new TreeNode(sysConfig.getValue(), sysConfig.getName(),"");
				treeNode
				.setTreeNodeURL("MatchedPapers.do?act=list&mptId="
						+ sysConfig.getValue()+"&scId="+sysConfig.getValue());//+"&state="+function.getState()
				treeNodes.add(treeNode);
			}
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersType po = new MatchedPapersType();
			Session ses = HibernateSessionFactory.openSession();
	...
	*更新
	*/
	public void update(MatchedPapersTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersType po = (MatchedPapersType) session.load(MatchedPapersType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersType) ses.load(MatchedPapersType.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersType(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersType po = (MatchedPapersType) ses.load(MatchedPapersType.class, ID);
			MatchedPapersTypeActionForm form = new MatchedPapersTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersType as MatchedPapersType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersType.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersType as MatchedPapersType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersType as MatchedPapersType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersType as MatchedPapersType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
