...
public class MatchedPapersMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapers.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapers po = new MatchedPapers();
	...
	*更新
	*/
	public void update(MatchedPapersActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapers po = (MatchedPapers) session.load(MatchedPapers.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapers) ses.load(MatchedPapers.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapers(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapers po = (MatchedPapers) ses.load(MatchedPapers.class, ID);
			MatchedPapersActionForm form = new MatchedPapersActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapers as MatchedPapers where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapers.sort asc ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapers as MatchedPapers where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapers> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapers as MatchedPapers where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapers.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapers as MatchedPapers where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
