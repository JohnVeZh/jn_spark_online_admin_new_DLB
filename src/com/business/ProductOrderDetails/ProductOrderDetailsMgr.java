...
import com.business.ProductOrderDetails.ProductOrderDetailsActionForm;


public class ProductOrderDetailsMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductOrderDetails.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductOrderDetailsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductOrderDetails po = new ProductOrderDetails();
	...
	*更新
	*/
	public void update(ProductOrderDetailsActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductOrderDetails po = (ProductOrderDetails) session.get(ProductOrderDetails.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			ProductOrderDetails po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductOrderDetails) ses.load(ProductOrderDetails.class, IDs[i]);
				//List list = ses.createFilter(po.getProductOrderDetails(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductOrderDetailsActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductOrderDetails po = (ProductOrderDetails) ses.get(ProductOrderDetails.class, ID);
			ProductOrderDetailsActionForm form = new ProductOrderDetailsActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductOrderDetails as ProductOrderDetails where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductOrderDetails.id   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderDetails as ProductOrderDetails where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductOrderDetails> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductOrderDetails as ProductOrderDetails where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductOrderDetails.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductOrderDetails as ProductOrderDetails where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
