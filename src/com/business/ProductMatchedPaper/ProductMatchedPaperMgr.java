...
import com.business.ProductMatchedPaper.ProductMatchedPaperActionForm;


public class ProductMatchedPaperMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(ProductMatchedPaper.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductMatchedPaperActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			ProductMatchedPaper po = new ProductMatchedPaper();
	...
	*更新
	*/
	public void update(ProductMatchedPaperActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			ProductMatchedPaper po = (ProductMatchedPaper) session.load(ProductMatchedPaper.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (ProductMatchedPaper) ses.load(ProductMatchedPaper.class, IDs[i]);
				//List list = ses.createFilter(po.getProductMatchedPaper(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductMatchedPaperActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			ProductMatchedPaper po = (ProductMatchedPaper) ses.load(ProductMatchedPaper.class, ID);
			ProductMatchedPaperActionForm form = new ProductMatchedPaperActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from ProductMatchedPaper as ProductMatchedPaper where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by ProductMatchedPaper.id   ";
			Query qCount = ses
					.createQuery("select count(id) from ProductMatchedPaper as ProductMatchedPaper where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<ProductMatchedPaper> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from ProductMatchedPaper as ProductMatchedPaper where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by ProductMatchedPaper.id desc";
			Query qCount = ses
					.createQuery("select count(id) from ProductMatchedPaper as ProductMatchedPaper where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
