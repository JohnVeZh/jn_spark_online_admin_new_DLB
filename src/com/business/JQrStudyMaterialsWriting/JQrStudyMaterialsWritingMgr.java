...
public class JQrStudyMaterialsWritingMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrStudyMaterialsWriting.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrStudyMaterialsWritingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrStudyMaterialsWriting po = new JQrStudyMaterialsWriting();
	...
	*更新
	*/
	public void update(JQrStudyMaterialsWritingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrStudyMaterialsWriting po = (JQrStudyMaterialsWriting) session.load(JQrStudyMaterialsWriting.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			JQrStudyMaterialsWriting po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrStudyMaterialsWriting) ses.load(JQrStudyMaterialsWriting.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrStudyMaterialsWriting(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrStudyMaterialsWritingActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrStudyMaterialsWriting po = (JQrStudyMaterialsWriting) ses.load(JQrStudyMaterialsWriting.class, ID);
			JQrStudyMaterialsWritingActionForm form = new JQrStudyMaterialsWritingActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrStudyMaterialsWriting as JQrStudyMaterialsWriting where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrStudyMaterialsWriting.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrStudyMaterialsWriting as JQrStudyMaterialsWriting where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrStudyMaterialsWriting> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrStudyMaterialsWriting as JQrStudyMaterialsWriting where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrStudyMaterialsWriting.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrStudyMaterialsWriting as JQrStudyMaterialsWriting where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
