...
public class NetworkVideoCatalogVideoMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NetworkVideoCatalogVideo.class);
	...
	public List getFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			NetworkVideoCatalog function = null;
			TreeNode treeNode = null;
			
			Collection queryConds = new ArrayList();
			Session ses = HibernateSessionFactory.openSession();
			String queryString = "from NetworkVideoCatalog where nvId = '"+parentId+"' ";
			...
			while (tempArray.hasNext()) {
				function =  (NetworkVideoCatalog)tempArray.next();
				treeNode = new TreeNode(function.getId(),function.getcName() , function.getParentId().equals("FFFFFF") ? "" : function.getParentId());
				treeNode
						.setTreeNodeURL("NetworkVideoCatalogVideo.do?act=list&nvcId="
								+ function.getId()+"&nvId="+parentId);//+"&state="+function.getState()
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NetworkVideoCatalogVideoActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NetworkVideoCatalogVideo po = new NetworkVideoCatalogVideo();
			Session ses = HibernateSessionFactory.openSession();
	...
	*更新
	*/
	public void update(NetworkVideoCatalogVideoActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NetworkVideoCatalogVideo po = (NetworkVideoCatalogVideo) session.load(NetworkVideoCatalogVideo.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NetworkVideoCatalogVideo) ses.load(NetworkVideoCatalogVideo.class, IDs[i]);
				//List list = ses.createFilter(po.getNetworkVideoCatalogVideo(), "").list();
	 ...
	 * @throws Exception
	 */
	public NetworkVideoCatalogVideoActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NetworkVideoCatalogVideo po = (NetworkVideoCatalogVideo) ses.load(NetworkVideoCatalogVideo.class, ID);
			NetworkVideoCatalogVideoActionForm form = new NetworkVideoCatalogVideoActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NetworkVideoCatalogVideo as NetworkVideoCatalogVideo where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NetworkVideoCatalogVideo.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoCatalogVideo as NetworkVideoCatalogVideo where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NetworkVideoCatalogVideo> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NetworkVideoCatalogVideo as NetworkVideoCatalogVideo where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NetworkVideoCatalogVideo.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NetworkVideoCatalogVideo as NetworkVideoCatalogVideo where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
