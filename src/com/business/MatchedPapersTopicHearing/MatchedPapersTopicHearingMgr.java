...
public class MatchedPapersTopicHearingMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicHearing.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicHearingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicHearing po = new MatchedPapersTopicHearing();
	...
	*更新
	*/
	public void update(MatchedPapersTopicHearingActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicHearing po = (MatchedPapersTopicHearing) session.load(MatchedPapersTopicHearing.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicHearing) ses.load(MatchedPapersTopicHearing.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicHearing(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicHearingActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicHearing po = (MatchedPapersTopicHearing) ses.load(MatchedPapersTopicHearing.class, ID);
			MatchedPapersTopicHearingActionForm form = new MatchedPapersTopicHearingActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicHearing as MatchedPapersTopicHearing where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicHearing.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearing as MatchedPapersTopicHearing where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicHearing> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicHearing as MatchedPapersTopicHearing where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicHearing.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicHearing as MatchedPapersTopicHearing where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
