...
public class HomeworkCorrectingRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(HomeworkCorrectingRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(HomeworkCorrectingRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			HomeworkCorrectingRecord po = new HomeworkCorrectingRecord();
	...
	*更新
	*/
	public void update(HomeworkCorrectingRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			HomeworkCorrectingRecord po = (HomeworkCorrectingRecord) session.load(HomeworkCorrectingRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			HomeworkCorrectingRecord po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (HomeworkCorrectingRecord) ses.load(HomeworkCorrectingRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getHomeworkCorrectingRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public HomeworkCorrectingRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			HomeworkCorrectingRecord po = (HomeworkCorrectingRecord) ses.load(HomeworkCorrectingRecord.class, ID);
			HomeworkCorrectingRecordActionForm form = new HomeworkCorrectingRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from HomeworkCorrectingRecord as HomeworkCorrectingRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by HomeworkCorrectingRecord.id   ";
			Query qCount = ses
					.createQuery("select count(id) from HomeworkCorrectingRecord as HomeworkCorrectingRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<HomeworkCorrectingRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from HomeworkCorrectingRecord as HomeworkCorrectingRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by HomeworkCorrectingRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from HomeworkCorrectingRecord as HomeworkCorrectingRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
