...
public class PreferentialCodeUseMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(PreferentialCodeUse.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(PreferentialCodeUseActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			PreferentialCodeUse po = new PreferentialCodeUse();
	...
	*更新
	*/
	public void update(PreferentialCodeUseActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			PreferentialCodeUse po = (PreferentialCodeUse) session.load(PreferentialCodeUse.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			PreferentialCodeUse po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (PreferentialCodeUse) ses.load(PreferentialCodeUse.class, IDs[i]);
				//List list = ses.createFilter(po.getPreferentialCodeUse(), "").list();
	 ...
	 * @throws Exception
	 */
	public PreferentialCodeUseActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			PreferentialCodeUse po = (PreferentialCodeUse) ses.load(PreferentialCodeUse.class, ID);
			PreferentialCodeUseActionForm form = new PreferentialCodeUseActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from PreferentialCodeUse as PreferentialCodeUse where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by PreferentialCodeUse.createtime desc  ";
			Query qCount = ses
					.createQuery("select count(id) from PreferentialCodeUse as PreferentialCodeUse where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<PreferentialCodeUse> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from PreferentialCodeUse as PreferentialCodeUse where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by PreferentialCodeUse.id desc";
			Query qCount = ses
					.createQuery("select count(id) from PreferentialCodeUse as PreferentialCodeUse where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
