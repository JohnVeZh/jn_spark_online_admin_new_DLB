...
public class MatchedWrongRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedWrongRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedWrongRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedWrongRecord po = new MatchedWrongRecord();
	...
	*更新
	*/
	public void update(MatchedWrongRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedWrongRecord po = (MatchedWrongRecord) session.load(MatchedWrongRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedWrongRecord) ses.load(MatchedWrongRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedWrongRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedWrongRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedWrongRecord po = (MatchedWrongRecord) ses.load(MatchedWrongRecord.class, ID);
			MatchedWrongRecordActionForm form = new MatchedWrongRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedWrongRecord as MatchedWrongRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedWrongRecord.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedWrongRecord as MatchedWrongRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedWrongRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedWrongRecord as MatchedWrongRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedWrongRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedWrongRecord as MatchedWrongRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
