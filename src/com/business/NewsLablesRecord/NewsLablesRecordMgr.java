...
public class NewsLablesRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewsLablesRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewsLablesRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewsLablesRecord po = new NewsLablesRecord();
	...
	*更新
	*/
	public void update(NewsLablesRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewsLablesRecord po = (NewsLablesRecord) session.load(NewsLablesRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewsLablesRecord po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewsLablesRecord) ses.load(NewsLablesRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getNewsLablesRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewsLablesRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewsLablesRecord po = (NewsLablesRecord) ses.load(NewsLablesRecord.class, ID);
			NewsLablesRecordActionForm form = new NewsLablesRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewsLablesRecord as NewsLablesRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewsLablesRecord.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewsLablesRecord as NewsLablesRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewsLablesRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewsLablesRecord as NewsLablesRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewsLablesRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewsLablesRecord as NewsLablesRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
