...
import com.business.MatchedPapersUser.MatchedPapersUser;
import com.business.MatchedPapersUser.MatchedPapersUserActionForm;


public class MatchedPapersUserMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersUser.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersUserActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersUser po = new MatchedPapersUser();
	...
	*更新
	*/
	public void update(MatchedPapersUserActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersUser po = (MatchedPapersUser) session.load(MatchedPapersUser.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersUser) ses.load(MatchedPapersUser.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersUser(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersUserActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersUser po = (MatchedPapersUser) ses.load(MatchedPapersUser.class, ID);
			MatchedPapersUserActionForm form = new MatchedPapersUserActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersUser as MatchedPapersUser where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersUser.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersUser as MatchedPapersUser where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersUser> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersUser as MatchedPapersUser where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersUser.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersUser as MatchedPapersUser where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
