...
public class NewLablesMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewLables.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewLablesActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewLables po = new NewLables();
	...
	*更新
	*/
	public void update(NewLablesActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewLables po = (NewLables) session.load(NewLables.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NewLables) ses.load(NewLables.class, IDs[i]);
				//List list = ses.createFilter(po.getNewLables(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewLablesActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewLables po = (NewLables) ses.load(NewLables.class, ID);
			NewLablesActionForm form = new NewLablesActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			String hq =null;
			if("1".equals(sort)){
				 hq = "from NewLables as NewLables where 1=1 "
						+ QueryHelper.toAndQuery(queryConds)
						+ " order by NewLables.sort    ";
			}else if("2".equals(sort)){
				hq = "from NewLables as NewLables where 1=1 "
						+ QueryHelper.toAndQuery(queryConds)
						+ " order by NewLables.sort desc  ";
			}else{
				hq = "from NewLables as NewLables where 1=1 "
						+ QueryHelper.toAndQuery(queryConds)
						+ " order by NewLables.id  ";
			}
			
			Query qCount = ses
					.createQuery("select count(id) from NewLables as NewLables where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewLables> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewLables as NewLables where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewLables.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewLables as NewLables where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
