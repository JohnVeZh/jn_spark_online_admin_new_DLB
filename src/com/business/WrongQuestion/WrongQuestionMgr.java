...
import com.business.WrongQuestion.WrongQuestion;
import com.business.WrongQuestion.WrongQuestionActionForm;


public class WrongQuestionMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(WrongQuestion.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(WrongQuestionActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			WrongQuestion po = new WrongQuestion();
	...
	*更新
	*/
	public void update(WrongQuestionActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			WrongQuestion po = (WrongQuestion) session.load(WrongQuestion.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (WrongQuestion) ses.load(WrongQuestion.class, IDs[i]);
				//List list = ses.createFilter(po.getWrongQuestion(), "").list();
	 ...
	 * @throws Exception
	 */
	public WrongQuestionActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			WrongQuestion po = (WrongQuestion) ses.load(WrongQuestion.class, ID);
			WrongQuestionActionForm form = new WrongQuestionActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from WrongQuestion as WrongQuestion where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by WrongQuestion.id   ";
			Query qCount = ses
					.createQuery("select count(id) from WrongQuestion as WrongQuestion where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<WrongQuestion> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from WrongQuestion as WrongQuestion where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by WrongQuestion.id desc";
			Query qCount = ses
					.createQuery("select count(id) from WrongQuestion as WrongQuestion where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
