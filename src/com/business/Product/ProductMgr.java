...
public class ProductMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(Product.class);
	...
	public List getFuntree(String parentId) throws SystemException, Exception {
		...
		try {
			...
			while (tempArray.hasNext()) {
				...
				treeNode
						.setTreeNodeURL("Product.do?act=list&pTypeId="
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(ProductActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(ProductActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			Product po = (Product) session.load(Product.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (Product) ses.load(Product.class, IDs[i]);
				//List list = ses.createFilter(po.getProduct(), "").list();
	 ...
	 * @throws Exception
	 */
	public ProductActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			Product po = (Product) ses.load(Product.class, ID);
			ProductActionForm form = new ProductActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from Product as Product where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by Product.batchTime,Product.pCreatetime desc  ";
			Query qCount = ses
					.createQuery("select count(id) from Product as Product where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<Product> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from Product as Product where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by Product.id desc";
			Query qCount = ses
					.createQuery("select count(id) from Product as Product where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
