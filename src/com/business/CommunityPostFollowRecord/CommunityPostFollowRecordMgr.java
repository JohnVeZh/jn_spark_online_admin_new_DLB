...
public class CommunityPostFollowRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(CommunityPostFollowRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(CommunityPostFollowRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			CommunityPostFollowRecord po = new CommunityPostFollowRecord();
	...
	*更新
	*/
	public void update(CommunityPostFollowRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			CommunityPostFollowRecord po = (CommunityPostFollowRecord) session.load(CommunityPostFollowRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (CommunityPostFollowRecord) ses.load(CommunityPostFollowRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getCommunityPostFollowRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public CommunityPostFollowRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			CommunityPostFollowRecord po = (CommunityPostFollowRecord) ses.load(CommunityPostFollowRecord.class, ID);
			CommunityPostFollowRecordActionForm form = new CommunityPostFollowRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from CommunityPostFollowRecord as CommunityPostFollowRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by CommunityPostFollowRecord.createtime desc    ";
			Query qCount = ses
					.createQuery("select count(id) from CommunityPostFollowRecord as CommunityPostFollowRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<CommunityPostFollowRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from CommunityPostFollowRecord as CommunityPostFollowRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by CommunityPostFollowRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from CommunityPostFollowRecord as CommunityPostFollowRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
