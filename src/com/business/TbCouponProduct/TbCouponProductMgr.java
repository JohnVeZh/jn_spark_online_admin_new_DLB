 ...
 */
public class TbCouponProductMgr extends AbstractHibernateDAO {
    private static LogUtils logger = LogUtils.getLogger(PreferentialCodeProduct.class);
     ...
     *           要抛出的异常 可定义多个
     */
    public void add(TbCouponProductActionForm vo) throws SystemException, Exception {
        Transaction tx = null;

        try {
            TbCouponProduct po = new TbCouponProduct();
     ...
     *更新
     */
    public void update(TbCouponProductActionForm vo) throws SystemException, Exception {
        Transaction tx = null;
        try {

            Session session = HibernateSessionFactory.openSession();
            tx = session.beginTransaction();
            PreferentialCodeProduct po = (PreferentialCodeProduct) session.load(PreferentialCodeProduct.class, vo.getId());
            super.copyProperties(po, vo);
            session.update(po);
            tx.commit();
        } catch (Exception e) {
    ...
    public void delete(String[] IDs) throws SystemException, Exception {
        ...
        try {
            Session ses = HibernateSessionFactory.openSession();
            tx = ses.beginTransaction();
            PreferentialCodeProduct po = null;
            for (int i = 0; i < IDs.length; i++) {
                po = (PreferentialCodeProduct) ses.load(PreferentialCodeProduct.class, IDs[i]);
                //List list = ses.createFilter(po.getPreferentialCodeProduct(), "").list();
     ...
     * @throws Exception
     */
    public TbCouponProductActionForm view(String ID) throws SystemException, Exception {
        try {
            Session ses = HibernateSessionFactory.openSession();
            PreferentialCodeProduct po = (PreferentialCodeProduct) ses.load(PreferentialCodeProduct.class, ID);
            TbCouponProductActionForm form = new TbCouponProductActionForm();
    ...
    public ListContainer list(Collection queryConds, int currentPageInt,
        ...
        try {
            ...
            // 查询对象
            String hq = "from TbCouponProduct as TbCouponProduct where 1=1 "
                    + QueryHelper.toAndQuery(queryConds)
                    + " order by TbCouponProduct.id desc   ";
            Query qCount = ses
                    .createQuery("select count(id) from TbCouponProduct as TbCouponProduct where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
     ...
     */
    public List<PreferentialCodeProduct> mlist(Collection queryConds,MobilePage mp) throws SystemException,
            Exception {
        try {
            // 在数据准备完成后，获取hiernate会话
            Session ses = HibernateSessionFactory.openSession();
            ...
            // 保存对象
            String hq = "from PreferentialCodeProduct as PreferentialCodeProduct where 1=1 "
                    + QueryHelper.toAndQuery(queryConds) + " order by PreferentialCodeProduct.id desc";
            Query qCount = ses
                    .createQuery("select count(id) from PreferentialCodeProduct as PreferentialCodeProduct where 1=1 "
                            + QueryHelper.toAndQuery(queryConds));
    ...
    public void addProducts(String deleteSql,String insertSql) throws SystemException, Exception {
        Transaction tx = null;
        try {
            Session session = HibernateSessionFactory.openSession();
            tx = session.beginTransaction();
