...
public class NewQuestionsTypePapersMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsTypePapers.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsTypePapersActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsTypePapers po = new NewQuestionsTypePapers();
	...
	*更新
	*/
	public void update(NewQuestionsTypePapersActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsTypePapers po = (NewQuestionsTypePapers) session.load(NewQuestionsTypePapers.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsTypePapers) ses.load(NewQuestionsTypePapers.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsTypePapers(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsTypePapersActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsTypePapers po = (NewQuestionsTypePapers) ses.load(NewQuestionsTypePapers.class, ID);
			NewQuestionsTypePapersActionForm form = new NewQuestionsTypePapersActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsTypePapers as NewQuestionsTypePapers where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsTypePapers.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsTypePapers as NewQuestionsTypePapers where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsTypePapers> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsTypePapers as NewQuestionsTypePapers where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsTypePapers.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsTypePapers as NewQuestionsTypePapers where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
