...
public class JQrCodeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JQrCode.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JQrCodeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JQrCode po = new JQrCode();
	...
	*更新
	*/
	public void update(JQrCodeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JQrCode po = (JQrCode) session.load(JQrCode.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JQrCode) ses.load(JQrCode.class, IDs[i]);
				//List list = ses.createFilter(po.getJQrCode(), "").list();
	 ...
	 * @throws Exception
	 */
	public JQrCodeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JQrCode po = (JQrCode) ses.load(JQrCode.class, ID);
			JQrCodeActionForm form = new JQrCodeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JQrCode as JQrCode where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JQrCode.id   ";
			Query qCount = ses
					.createQuery("select count(id) from JQrCode as JQrCode where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JQrCode> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JQrCode as JQrCode where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JQrCode.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JQrCode as JQrCode where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
