...
public class OnlineUserMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(OnlineUser.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(OnlineUserActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			OnlineUser po = new OnlineUser();
	...
	*更新
	*/
	public void update(OnlineUserActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			OnlineUser po = (OnlineUser) session.load(OnlineUser.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (OnlineUser) ses.load(OnlineUser.class, IDs[i]);
				//List list = ses.createFilter(po.getOnlineUser(), "").list();
	 ...
	 * @throws Exception
	 */
	public OnlineUserActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			OnlineUser po = (OnlineUser) ses.load(OnlineUser.class, ID);
			OnlineUserActionForm form = new OnlineUserActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from OnlineUser as OnlineUser where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by OnlineUser.id   ";
			Query qCount = ses
					.createQuery("select count(id) from OnlineUser as OnlineUser where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<OnlineUser> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from OnlineUser as OnlineUser where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by OnlineUser.id desc";
			Query qCount = ses
					.createQuery("select count(id) from OnlineUser as OnlineUser where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
