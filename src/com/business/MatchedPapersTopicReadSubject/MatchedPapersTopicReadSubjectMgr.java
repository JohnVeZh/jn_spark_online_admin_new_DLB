...
public class MatchedPapersTopicReadSubjectMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(MatchedPapersTopicReadSubject.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(MatchedPapersTopicReadSubjectActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			MatchedPapersTopicReadSubject po = new MatchedPapersTopicReadSubject();
	...
	*更新
	*/
	public void update(MatchedPapersTopicReadSubjectActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			MatchedPapersTopicReadSubject po = (MatchedPapersTopicReadSubject) session.load(MatchedPapersTopicReadSubject.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			MatchedPapersTopicReadSubject po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (MatchedPapersTopicReadSubject) ses.load(MatchedPapersTopicReadSubject.class, IDs[i]);
				//List list = ses.createFilter(po.getMatchedPapersTopicReadSubject(), "").list();
	 ...
	 * @throws Exception
	 */
	public MatchedPapersTopicReadSubjectActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			MatchedPapersTopicReadSubject po = (MatchedPapersTopicReadSubject) ses.load(MatchedPapersTopicReadSubject.class, ID);
			MatchedPapersTopicReadSubjectActionForm form = new MatchedPapersTopicReadSubjectActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from MatchedPapersTopicReadSubject as MatchedPapersTopicReadSubject where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by MatchedPapersTopicReadSubject.id   ";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicReadSubject as MatchedPapersTopicReadSubject where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<MatchedPapersTopicReadSubject> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from MatchedPapersTopicReadSubject as MatchedPapersTopicReadSubject where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by MatchedPapersTopicReadSubject.id desc";
			Query qCount = ses
					.createQuery("select count(id) from MatchedPapersTopicReadSubject as MatchedPapersTopicReadSubject where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
