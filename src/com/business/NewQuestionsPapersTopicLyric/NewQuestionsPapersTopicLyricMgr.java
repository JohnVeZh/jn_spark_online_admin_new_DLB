...
public class NewQuestionsPapersTopicLyricMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsPapersTopicLyric.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsPapersTopicLyricActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsPapersTopicLyric po = new NewQuestionsPapersTopicLyric();
	...
	*更新
	*/
	public void update(NewQuestionsPapersTopicLyricActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsPapersTopicLyric po = (NewQuestionsPapersTopicLyric) session.load(NewQuestionsPapersTopicLyric.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewQuestionsPapersTopicLyric po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsPapersTopicLyric) ses.load(NewQuestionsPapersTopicLyric.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsPapersTopicLyric(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsPapersTopicLyricActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsPapersTopicLyric po = (NewQuestionsPapersTopicLyric) ses.load(NewQuestionsPapersTopicLyric.class, ID);
			NewQuestionsPapersTopicLyricActionForm form = new NewQuestionsPapersTopicLyricActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsPapersTopicLyric as NewQuestionsPapersTopicLyric where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsPapersTopicLyric.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopicLyric as NewQuestionsPapersTopicLyric where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsPapersTopicLyric> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsPapersTopicLyric as NewQuestionsPapersTopicLyric where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsPapersTopicLyric.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopicLyric as NewQuestionsPapersTopicLyric where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
