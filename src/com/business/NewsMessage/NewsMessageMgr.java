...
import com.business.NewsMessage.NewsMessage;
import com.business.NewsMessage.NewsMessageActionForm;


public class NewsMessageMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewsMessage.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewsMessageActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewsMessage po = new NewsMessage();
	...
	*更新
	*/
	public void update(NewsMessageActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewsMessage po = (NewsMessage) session.load(NewsMessage.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (NewsMessage) ses.load(NewsMessage.class, IDs[i]);
				//List list = ses.createFilter(po.getNewsMessage(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewsMessageActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewsMessage po = (NewsMessage) ses.load(NewsMessage.class, ID);
			NewsMessageActionForm form = new NewsMessageActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
//			String hq = "from NewsMessage as NewsMessage where 1=1 "
//					+ QueryHelper.toAndQuery(queryConds)
//					+ " order by NewsMessage.createtime desc   ";
//			Query qCount = ses
//					.createQuery("select count(id) from NewsMessage as NewsMessage where 1=1 "
//							+ QueryHelper.toAndQuery(queryConds));
			String sql = "SELECT nm.id, nm.news_id, nm.user_id, cast(nm.content as char(500)), nm.createtime," +
					" nm.comments_id, nm.is_del, nm.like_num, nm.is_top, nm.top_time," +
					" users.username, users.mobile, news.title FROM news_message nm" +
					" left JOIN news ON nm.news_id = news.id left JOIN users ON nm.user_id = users.id" +
					" where nm.is_del = '0' "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by nm.top_time desc, nm.createtime desc   ";
			Query qCount = ses
					.createSQLQuery("select count(1)  FROM news_message nm" +
							" left JOIN news ON nm.news_id = news.id left JOIN users ON nm.user_id = users.id" +
							" where nm.is_del = '0' "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewsMessage> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewsMessage as NewsMessage where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewsMessage.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewsMessage as NewsMessage where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
