...
import com.business.JCommonType.JCommonTypeActionForm;


public class JCommonTypeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(JCommonType.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(JCommonTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			JCommonType po = new JCommonType();
	...
	*更新
	*/
	public void update(JCommonTypeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			JCommonType po = (JCommonType) session.load(JCommonType.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (JCommonType) ses.load(JCommonType.class, IDs[i]);
				//List list = ses.createFilter(po.getJCommonType(), "").list();
	 ...
	 * @throws Exception
	 */
	public JCommonTypeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			JCommonType po = (JCommonType) ses.load(JCommonType.class, ID);
			JCommonTypeActionForm form = new JCommonTypeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from JCommonType as JCommonType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by JCommonType.sort   ";
			Query qCount = ses
					.createQuery("select count(id) from JCommonType as JCommonType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<JCommonType> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from JCommonType as JCommonType where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by JCommonType.id desc";
			Query qCount = ses
					.createQuery("select count(id) from JCommonType as JCommonType where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
