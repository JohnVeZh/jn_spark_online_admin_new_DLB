...
public class NewQuestionsPapersTopicMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(NewQuestionsPapersTopic.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(NewQuestionsPapersTopicActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			NewQuestionsPapersTopic po = new NewQuestionsPapersTopic();
	...
	*更新
	*/
	public void update(NewQuestionsPapersTopicActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			NewQuestionsPapersTopic po = (NewQuestionsPapersTopic) session.load(NewQuestionsPapersTopic.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			NewQuestionsPapersTopic po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (NewQuestionsPapersTopic) ses.load(NewQuestionsPapersTopic.class, IDs[i]);
				//List list = ses.createFilter(po.getNewQuestionsPapersTopic(), "").list();
	 ...
	 * @throws Exception
	 */
	public NewQuestionsPapersTopicActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			NewQuestionsPapersTopic po = (NewQuestionsPapersTopic) ses.load(NewQuestionsPapersTopic.class, ID);
			NewQuestionsPapersTopicActionForm form = new NewQuestionsPapersTopicActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from NewQuestionsPapersTopic as NewQuestionsPapersTopic where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by NewQuestionsPapersTopic.id   ";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopic as NewQuestionsPapersTopic where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<NewQuestionsPapersTopic> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from NewQuestionsPapersTopic as NewQuestionsPapersTopic where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by NewQuestionsPapersTopic.id desc";
			Query qCount = ses
					.createQuery("select count(id) from NewQuestionsPapersTopic as NewQuestionsPapersTopic where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
