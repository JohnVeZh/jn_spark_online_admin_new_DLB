...
import com.easecom.system.model.SysNoticeRecord;
import com.easecom.system.web.SysNoticeRecordActionForm;



public class SysNoticeRecordMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(SysNoticeRecord.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(SysNoticeRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		 
		try {
			SysNoticeRecord po = new SysNoticeRecord();
	...
	*更新
	*/
	public void update(SysNoticeRecordActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			SysNoticeRecord po = (SysNoticeRecord) session.load(SysNoticeRecord.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			Session ses = HibernateSessionFactory.openSession();
			tx = ses.beginTransaction();
			SysNoticeRecord po = null;
			for (int i = 0; i < IDs.length; i++) {
				po = (SysNoticeRecord) ses.load(SysNoticeRecord.class, IDs[i]);
				//List list = ses.createFilter(po.getSysNoticeRecord(), "").list();
	 ...
	 * @throws Exception
	 */
	public SysNoticeRecordActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			SysNoticeRecord po = (SysNoticeRecord) ses.load(SysNoticeRecord.class, ID);
			SysNoticeRecordActionForm form = new SysNoticeRecordActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from SysNoticeRecord as SysNoticeRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by SysNoticeRecord.id desc   ";
			Query qCount = ses
					.createQuery("select count(id) from SysNoticeRecord as SysNoticeRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<SysNoticeRecord> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from SysNoticeRecord as SysNoticeRecord where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by SysNoticeRecord.id desc";
			Query qCount = ses
					.createQuery("select count(id) from SysNoticeRecord as SysNoticeRecord where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
