...
import com.easecom.system.web.SysNoticeActionForm;



public class SysNoticeMgr extends AbstractHibernateDAO {
	private static LogUtils logger = LogUtils.getLogger(SysNotice.class);
	 ...
	 *           要抛出的异常 可定义多个
	 */
	public void add(SysNoticeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
	...
	*更新
	*/
	public void update(SysNoticeActionForm vo) throws SystemException, Exception {
		Transaction tx = null;
		try {
		
			Session session = HibernateSessionFactory.openSession();
			tx = session.beginTransaction();
			SysNotice po = (SysNotice) session.load(SysNotice.class, vo.getId());
			super.copyProperties(po, vo);
			session.update(po);
			tx.commit();
		} catch (Exception e) {
	...
	public void delete(String[] IDs) throws SystemException, Exception {
		...
		try {
			...
			for (int i = 0; i < IDs.length; i++) {
				po = (SysNotice) ses.load(SysNotice.class, IDs[i]);
				//List list = ses.createFilter(po.getSysNotice(), "").list();
	 ...
	 * @throws Exception
	 */
	public SysNoticeActionForm view(String ID) throws SystemException, Exception {
		try {
			Session ses = HibernateSessionFactory.openSession();
			SysNotice po = (SysNotice) ses.load(SysNotice.class, ID);
			SysNoticeActionForm form = new SysNoticeActionForm();
	...
	public ListContainer list(Collection queryConds, int currentPageInt,
		...
		try {
			...
			// 查询对象
			String hq = "from SysNotice as SysNotice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds)
					+ " order by SysNotice.createtime desc  ";
			Query qCount = ses
					.createQuery("select count(id) from SysNotice as SysNotice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
...
/**
	 ...
	 */
	public List<SysNotice> mlist(Collection queryConds,MobilePage mp) throws SystemException,
			Exception {
		try {
			// 在数据准备完成后，获取hiernate会话
			Session ses = HibernateSessionFactory.openSession();
			...
			// 保存对象
			String hq = "from SysNotice as SysNotice where 1=1 "
					+ QueryHelper.toAndQuery(queryConds) + " order by SysNotice.id desc";
			Query qCount = ses
					.createQuery("select count(id) from SysNotice as SysNotice where 1=1 "
							+ QueryHelper.toAndQuery(queryConds));
